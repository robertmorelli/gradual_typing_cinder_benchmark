...
def weakest_of(s1: Strength, s2):
    return cast(Strength, s1 if s1.strength > cast(Strength, s2).strength else cast(Strength, s2))
...
class Strength:
    ...
    def next_weaker(self):
        return cast(Strength, cast(Strength, CheckedList[Strength](STRENGTHS)[box(self.strength)]))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            self.v2.value = box(int64(int64(cast(Variable, self.v1).value) * int64(cast(Variable, self.scale).value) + int64(cast(Variable, self.offset).value)))
        else:
            cast(Variable, self.v1).value = box(int64((int64(self.v2.value) - int64(cast(Variable, self.offset).value)) / int64(cast(Variable, self.scale).value)))
