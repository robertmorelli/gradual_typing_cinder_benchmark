...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if cast(Variable, self.v1).mark == int64(mark):
            if cast(Variable, self.v2).mark != int64(mark) and stronger(self.strength, cast(Strength, cast(Variable, self.v2).walk_strength)):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if cast(Variable, self.v2).mark == int64(mark):
            if cast(Variable, self.v1).mark != int64(mark) and stronger(self.strength, cast(Strength, cast(Variable, self.v1).walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        if weaker(cast(Strength, cast(Variable, self.v1).walk_strength), cast(Strength, cast(Variable, self.v2).walk_strength)):
            if stronger(self.strength, cast(Strength, cast(Variable, self.v1).walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src, scale, offset: Variable, dest: Variable, strength) -> None:
...
class Planner(object):
    ...
    def make_plan(self, sources):
        ...
        while clen(todo):
            c = todo.pop(0)
            if cast(Variable, c.output()).mark != mark and cbool(c.inputs_known(mark)):
