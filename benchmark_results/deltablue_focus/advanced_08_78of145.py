...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64):
        if cast(Variable, self.my_output).mark != mark and stronger(self.strength, cast(Strength, cast(Variable, self.my_output).walk_strength)):
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64):
        if self.v1.mark == mark:
            if cast(Variable, self.v2).mark != mark and stronger(self.strength, cast(Strength, cast(Variable, self.v2).walk_strength)):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if cast(Variable, self.v2).mark == mark:
            if self.v1.mark != mark and stronger(self.strength, cast(Strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
