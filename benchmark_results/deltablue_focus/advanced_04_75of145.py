...
def weakest_of(s1, s2: Strength) -> Strength:
    return cast(Strength, s1) if cast(Strength, s1).strength > s2.strength else s2
...
class Strength:
    ...
    def next_weaker(self) -> Strength:
        return cast(Strength, CheckedList[Strength](STRENGTHS)[box(self.strength)])
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        ...
        if int64(cast(Variable, self.v2).mark) == int64(mark):
            if int64(cast(Variable, self.v1).mark) != int64(mark) and stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v1).walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
    ...
    def inputs_known(self, mark: int64):
        i = self.input()
        return box(cbool(int64(i.mark) == mark or i.stay or cbool(cast(Constraint | None, i.determined_by) is None)))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            cast(Variable, self.v2).value = box(int64(int64(cast(Variable, self.v1).value) * int64(self.scale.value) + int64(cast(Variable, self.offset).value)))
        else:
            cast(Variable, self.v1).value = box(int64((int64(cast(Variable, self.v2).value) - int64(cast(Variable, self.offset).value)) / int64(self.scale.value)))
...
class EqualityConstraint(BinaryConstraint):
    ...
    def execute(self):
        cast(Variable, self.output()).value = box(int64(int64(self.input().value)))
...
class Planner(object):
    ...
    def incremental_add(self, constraint):
        mark = self.new_mark()
        overridden = cast(Constraint | None, cast(Constraint, constraint).satisfy(mark))
    ...
    def make_plan(self, sources):
        ...
        while clen(todo):
            c = todo.pop(0)
            if int64(cast(Variable, c.output()).mark) != mark and cbool(c.inputs_known(mark)):
...
def projection_test(n) -> None:
    ...
    i = 0
    while i < int64(n) - 1:
        if int64(cast(Variable, dests[box(i)]).value) != i * 5 + 1000:
            print('Projection 3 failed')
        i = i + 1
    change(offset, 2000)
    i = 0
    while i < int64(n) - 1:
        if int64(cast(Variable, dests[box(i)]).value) != i * 5 + 2000:
