...
class Strength(object):
    ...
    @classmethod
    def stronger(cls, s1: Strength, s2) -> bool:
        return s1.strength < cast(Strength, s2).strength

    @classmethod
    def weaker(cls, s1: Strength, s2) -> bool:
        return s1.strength > cast(Strength, s2).strength

    @classmethod
    def weakest_of(cls, s1: Strength, s2: Strength):
        if cls.weaker(s1, s2):
            return cast(Strength, s1)
        return cast(Strength, s2)
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int):
    ...
    def input(self):
        if self.direction == Direction.FORWARD:
            return cast(Variable, self.v1)
        return cast(Variable, self.v2)
    ...
    def recalculate(self):
        ...
        out = self.output()
        out.walk_strength = cast(Strength, Strength.weakest_of(self.strength, ihn.walk_strength))
        out.stay = ihn.stay
        if out.stay:
            self.execute()
    ...
    def inputs_known(self, mark: int):
...
class Variable(object):
    ...
    def __init__(self, name, initial_value: int=0):
    ...
    def remove_constraint(self, constraint):
        self.constraints.remove(cast(EditConstraint, constraint))
        if self.determined_by == cast(EditConstraint, constraint):
            self.determined_by = None
...
class Planner(object):
    ...
    def incremental_remove(self, constraint: Constraint):
        ...
        unsatisfied = cast(OrderedCollection, self.remove_propagate_from(out))
    ...
    def make_plan(self, sources):
        mark = self.new_mark()
        plan = Plan()
        todo = cast(OrderedCollection, sources)
        while len(todo):
            c = todo.pop(0)
    ...
    def extract_plan_from_constraints(self, constraints) -> Plan:
        ...
        x = len(cast(OrderedCollection, constraints))
        i = 0
        while i < x:
            c = cast(OrderedCollection, constraints)[i]
            if c.is_input() and c.is_satisfied():
                sources.append(c)
            i = i + 1
        return cast(Plan, self.make_plan(sources))

    def add_propagate(self, c: Constraint, mark: int):
    ...
    def remove_propagate_from(self, out: Variable):
        ...
        while len(todo):
            ...
            while i < x:
                ...
                i = i + 1
        return cast(OrderedCollection, unsatisfied)

    def add_constraints_consuming_to(self, v: Variable, coll: OrderedCollection):
        determining = v.determined_by
        cc = v.constraints
        x = len(cc)
        i = 0
