...
class Strength(object):
    ...
    @classmethod
    def stronger(cls, s1, s2: Strength) -> bool:
        return cast(Strength, s1).strength < s2.strength

    @classmethod
    def weaker(cls, s1: Strength, s2: Strength):
        return s1.strength > s2.strength

    @classmethod
    def weakest_of(cls, s1, s2) -> Strength:
...
class Planner(object):
    ...
    def add_propagate(self, c, mark) -> bool:
    ...
    def remove_propagate_from(self, out) -> OrderedCollection:
        cast(Variable, out).determined_by = None
        cast(Variable, out).walk_strength = Strength.WEAKEST
    ...
    def add_constraints_consuming_to(self, v, coll) -> None:
        ...
        while i < x:
            c = cc[i]
            if c != determining and c.is_satisfied():
                cast(OrderedCollection, coll).append(c)
            i = i + 1
...
def change(v, new_value):
    planner = cast(Planner, get_planner())
    edit = EditConstraint(cast(Variable, v), Strength.PREFERRED)
    edits = OrderedCollection()
    edits.append(edit)
    plan = cast(Plan, planner.extract_plan_from_constraints(edits))
    i = 0
