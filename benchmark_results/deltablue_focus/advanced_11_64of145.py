...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if int64(self.my_output.mark) != int64(mark) and cbool(stronger(cast(Strength, self.strength), cast(Strength, self.my_output.walk_strength))):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def inputs_known(self, mark: int64):
        ...
        return box(cbool(int64(i.mark) == mark or cbool(i.stay) or cbool(cast(Constraint | None, i.determined_by) is None)))
