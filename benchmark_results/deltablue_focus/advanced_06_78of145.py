...
def weakest_of(s1: Strength, s2) -> Strength:
    return s1 if int64(s1.strength) > int64(cast(Strength, s2).strength) else cast(Strength, s2)
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if int64(cast(Variable, self.v1).mark) == int64(mark):
            if int64(self.v2.mark) != int64(mark) and cbool(stronger(cast(Strength, self.strength), cast(Strength, self.v2.walk_strength))):
