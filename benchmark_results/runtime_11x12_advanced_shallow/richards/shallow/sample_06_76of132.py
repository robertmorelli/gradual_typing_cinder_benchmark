...
class HandlerTaskRec(TaskRec):
    ...
    def workInAdd(self, p: Packet):
        ...
        return cast(Packet | None, cast(Packet | None, self.work_in))

    def deviceInAdd(self, p):
        self.device_in = cast(Packet, cast(Packet, p).append_to(cast(Packet | None, self.device_in)))
    ...
    def fn(self, pkt, r):
        ...
        if pkt is not None:
            if cast(Packet, pkt).kind == K_WORK:
                ...
                cast(Packet | None, cast(HandlerTaskRec, h).deviceInAdd(pkt))
        ...
        dev = cast(Packet | None, cast(HandlerTaskRec, h).device_in)
        if dev is None:
            return cast(Task, cast(Task, self.waitTask()))
...
class WorkTask(Task):
    ...
    def fn(self, pkt, r):
        ...
        while i < BUFSIZE:
            ...
            if w.count > 26:
                w.count = 1
            cast(Packet, pkt).data[i] = A + w.count - 1
            i = i + 1
        return cast(Task, self.qpkt(pkt))
...
def schedule():
    ...
    while t is not None:
        ...
        if cast(Task, t).isTaskHoldingOrWaiting():
            t = cast(Task | None, cast(Task, t).link)
        else:
            if tracing:
