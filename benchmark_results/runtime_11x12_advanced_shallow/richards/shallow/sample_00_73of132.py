...
class Packet(object):
    ...
    def __init__(self, l: Packet | None, i, k: int):
        self.link = l
        self.ident = i
        self.kind = k
        self.datum = 0
...
class HandlerTaskRec(TaskRec):
    ...
    def deviceInAdd(self, p: Packet):
        self.device_in = p.append_to(cast(Packet | None, self.device_in))
...
class Task(TaskState):
    ...
    def __init__(self, i, p, w, initialState, r):
        self.link = cast(Task | None, cast(TaskWorkArea, taskWorkArea).taskList)
        self.ident: int = i
        self.priority = p
        self.input = w
