...
class Task(TaskState):
    ...
    def release(self, i) -> Task:
        t = cast(Task, Task.findtcb(self, int64(i)))
    ...
    def qpkt(self, pkt) -> Task:
        t: Task = Task.findtcb(self, cast(Packet, pkt).ident)
...
class WorkTask(Task):
    ...
    def fn(self, pkt, r):
        ...
        while i < BUFSIZE:
            ...
            cast(List[int], cast(Packet, pkt).data)[i] = A + box(cast(WorkerTaskRec, w).count) - 1
            i = i + 1
        return cast(Task, self.qpkt(pkt))
