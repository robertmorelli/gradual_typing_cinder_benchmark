...
class Packet(object):
    ...
    def __init__(self, l, i: int64, k: int64):
...
class TaskState(object):
    ...
    def isTaskHoldingOrWaiting(self):
        return box(cbool(self.task_holding or (not self.packet_pending and cbool(self.task_waiting))))

    @inline
    def isWaitingWithPacket(self):
        return box(cbool(self.packet_pending and cbool(self.task_waiting) and (not self.task_holding)))
tracing = False
layout = 0
...
class Task(TaskState):
    ...
    def __init__(self, i, p: int64, w, initialState, r):
        ...
        self.task_waiting = box(cbool(cast(TaskState, initialState).isTaskWaiting()))
        self.task_holding = cbool(cast(TaskState, initialState).isTaskHolding())
        self.handle = r
    ...
    def release(self, i) -> Task:
        t: Task = cast(Task, Task.findtcb(self, int64(i)))
    ...
    def qpkt(self, pkt):
        t = Task.findtcb(self, cast(Packet, pkt).ident)
...
class DeviceTask(Task):
    ...
    def __init__(self, i, p: int64, w, s, r):
...
class WorkTask(Task):
    ...
    def fn(self, pkt, r):
        ...
        while i < BUFSIZE:
            ...
            if w.count > 26:
                w.count = 1
            cast(List[int], cast(Packet, pkt).data)[i] = A + box(w.count) - 1
            i = i + 1
        return cast(Task, cast(Task, self.qpkt(pkt)))
