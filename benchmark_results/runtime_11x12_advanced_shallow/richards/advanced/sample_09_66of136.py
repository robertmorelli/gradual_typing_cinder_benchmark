...
class TaskState(object):
    ...
    def isTaskHoldingOrWaiting(self):
        return box(cbool(self.task_holding or (not cbool(self.packet_pending) and self.task_waiting)))
...
class Task(TaskState):
    ...
    def __init__(self, i, p, w, initialState: TaskState, r):
    ...
    def qpkt(self, pkt) -> Task:
        t = cast(Task, Task.findtcb(self, cast(Packet, pkt).ident))
    ...
    def findtcb(self, id) -> Task:
        t = taskWorkArea.taskTab[int64(id)]
        return cast(Task, t)

class DeviceTask(Task):

    def __init__(self, i, p, w, s: TaskState, r):
