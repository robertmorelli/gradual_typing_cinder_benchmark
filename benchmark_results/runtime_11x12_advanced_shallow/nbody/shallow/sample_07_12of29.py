...
def report_energy(bodies: List[Body]=SYSTEM, pairs=PAIRS, e=0.0):
    for ((x1, y1, z1), v1, m1), ((x2, y2, z2), v2, m2) in pairs:
        dx = x1 - x2
        dy = y1 - y2
        dz = z1 - z2
...
def offset_momentum(ref, bodies: List[Body]=SYSTEM, px: float=0.0, py: float=0.0, pz: float=0.0) -> None:
    for r, [vx, vy, vz], m in bodies:
        px -= vx * m
        py -= vy * m
        pz -= vz * m
