...
def advance(dt: float, n: int, bodies=SYSTEM, pairs: List[Tuple[Body, Body]]=PAIRS):
    for i in range(n):
...
def offset_momentum(ref: Body, bodies: List[Body]=SYSTEM, px: float=0.0, py: float=0.0, pz: float=0.0):
    for r, [vx, vy, vz], m in bodies:
        px -= vx * m
        py -= vy * m
        pz -= vz * m
