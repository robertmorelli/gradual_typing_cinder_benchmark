...
def combinations(l) -> List[Tuple[T, T]]:
...
def advance(dt, n: int, bodies: List[Body]=SYSTEM, pairs=PAIRS) -> None:
    for i in range(n):
...
def offset_momentum(ref, bodies: List[Body]=SYSTEM, px=0.0, py: float=0.0, pz: float=0.0):
    for r, [vx, vy, vz], m in bodies:
        px -= vx * m
        py -= vy * m
        pz -= vz * m
