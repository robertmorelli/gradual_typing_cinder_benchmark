...
PAIRS = combinations(cast(CheckedList[Body], SYSTEM))
...
def advance(dt: double, n, bodies=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS):
    for i in range(n):
        ...
        for b1, b2 in pairs:
            ...
            mag = dt * (dx * dx + double(dy) * double(dy) + dz * dz) ** double(-1.5)
...
def report_energy(bodies: CheckedList[Body]=SYSTEM, pairs=PAIRS, e: double=0.0) -> double:
    ...
    for body in bodies:
        v = cast(Vector, body.v)
        e += double(body.mass) * (v.x * v.x + double(v.y) * double(v.y) + v.z * v.z) / 2.0
    return e
