...
def combinations(l: CheckedList[Body]):
    ...
    for x in range(len(l) - 1):
        ls = cast(CheckedList[Body], l[x + 1:])
        for y in ls:
            result.append((l[x], y))
    return result
...
def report_energy(bodies: CheckedList[Body]=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS, e: double=0.0) -> double:
    ...
    for b1, b2 in pairs:
        ...
        e -= cast(Body, b1).mass * cast(Body, b2).mass / (dx * dx + dy * dy + dz * dz) ** 0.5
    for body in bodies:
        v = cast(Vector, body.v)
        e += body.mass * (double(v.x) * double(v.x) + v.y * v.y + double(v.z) * double(v.z)) / 2.0
    return e

def offset_momentum(ref, bodies, px=0.0, py: double=0.0, pz: double=0.0):
    for body in bodies:
        v: Vector = cast(Vector, cast(Body, body).v)
