...
def advance(dt: double, n, bodies=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS):
    for i in range(n):
        for b1, b2 in pairs:
            ...
            mag = dt * (dx * dx + dy * dy + double(dz) * double(dz)) ** double(-1.5)
...
def report_energy(bodies: CheckedList[Body]=SYSTEM, pairs=PAIRS, e: double=0.0) -> double:
    ...
    for body in bodies:
        ...
        e += double(cast(Body, body).mass) * (v.x * v.x + v.y * v.y + v.z * v.z) / 2.0
    return e

def offset_momentum(ref: Body, bodies, px=0.0, py: double=0.0, pz=0.0):
    for body in bodies:
        ...
        py -= v.y * double(m)
        pz -= v.z * double(m)
    m = box(double(double(ref.mass)))
    v = cast(Vector, ref.v)
    v.x = double(px) / double(m)
    v.y = py / double(m)
    v.z = double(pz) / double(m)

def bench_nbody(loops: int, reference: str, iterations):
