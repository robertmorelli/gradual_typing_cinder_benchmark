...
class Vector:
    ...
    def __repr__(self):
        return f'Vector({(box(double(self.x)), box(double(self.y)), box(self.z))})'
...
def advance(dt: double, n, bodies=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS):
    for i in range(n):
        ...
        for b1, b2 in pairs:
            ...
            dy: double = double(pos1.y) - double(pos2.y)
            dz: double = pos1.z - pos2.z
            mag = dt * (double(dx) * double(dx) + dy * dy + dz * dz) ** double(-1.5)
...
def report_energy(bodies=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS, e=0.0) -> double:
    b2: Body
    body: Body
    for b1, b2 in pairs:
        pos1 = cast(Body, b1).pos
        ...
        dz = pos1.z - pos2.z
        e -= cast(Body, b1).mass * b2.mass / (dx * dx + dy * dy + dz * dz) ** 0.5
    for body in bodies:
        v = cast(Vector, body.v)
        e += body.mass * (double(v.x) * double(v.x) + double(v.y) * double(v.y) + v.z * v.z) / 2.0
    return double(e)
...
def bench_nbody(loops, reference: str, iterations: int):
    ...
    for _ in range_it:
        report_energy(cast(CheckedList[Body], SYSTEM), PAIRS)
