...
def advance(dt: double, n, bodies: CheckedList[Body]=SYSTEM, pairs=PAIRS):
    for i in range(n):
        ...
        for b1, b2 in pairs:
            ...
            mag = dt * (double(dx) * double(dx) + dy * dy + double(dz) * double(dz)) ** double(-1.5)
            b1m = cast(Body, b1).mass * mag
            b2m = b2.mass * mag
...
def report_energy(bodies: CheckedList[Body]=SYSTEM, pairs=PAIRS, e=0.0):
    ...
    for body in bodies:
        v = cast(Vector, body.v)
        e += body.mass * (double(v.x) * double(v.x) + v.y * v.y + double(v.z) * double(v.z)) / 2.0
    return box(double(double(e)))

def offset_momentum(ref: Body, bodies: CheckedList[Body], px: double=0.0, py=0.0, pz: double=0.0):
    body: Body
    for body in bodies:
        v: Vector = cast(Vector, body.v)
    ...
    v.y = double(py) / double(m)
    v.z = box(double(pz / double(m)))
