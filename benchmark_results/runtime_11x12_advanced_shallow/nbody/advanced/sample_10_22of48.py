...
class Vector:
    ...
    def __repr__(self):
        return f'Vector({(box(self.x), box(double(self.y)), box(double(self.z)))})'

class Body:

    def __init__(self, _pos, v: Vector, mass: double):
...
def report_energy(bodies: CheckedList[Body]=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS, e=0.0):
    b1: Body
    body: Body
    for b1, b2 in pairs:
        pos1 = b1.pos
        ...
        e -= double(b1.mass) * double(cast(Body, b2).mass) / (dx * dx + dy * dy + dz * dz) ** 0.5
    for body in bodies:
        v = cast(Vector, body.v)
        e += double(body.mass) * (v.x * v.x + double(v.y) * double(v.y) + double(v.z) * double(v.z)) / 2.0
    return box(double(double(e)))

def offset_momentum(ref: Body, bodies: CheckedList[Body], px=0.0, py=0.0, pz=0.0):
    for body in bodies:
