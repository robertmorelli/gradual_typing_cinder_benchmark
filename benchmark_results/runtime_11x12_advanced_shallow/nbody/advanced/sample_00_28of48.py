...
class Body:
    ...
    def __repr__(self):
        return f'Body({self.pos}, {self.v}, {box(double(self.mass))})'
...
def advance(dt: double, n, bodies: CheckedList[Body]=SYSTEM, pairs=PAIRS):
    for i in range(n):
        for b1, b2 in pairs:
            ...
            mag = dt * (dx * dx + double(dy) * double(dy) + double(dz) * double(dz)) ** double(-1.5)
...
def report_energy(bodies=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS, e=0.0):
    for b1, b2 in pairs:
        pos1 = cast(Body, b1).pos
        pos2 = cast(Body, b2).pos
        dx = pos1.x - pos2.x
    ...
    for body in bodies:
        v = cast(Body, body).v
        e += double(cast(Body, body).mass) * (v.x * v.x + v.y * v.y + double(v.z) * double(v.z)) / 2.0
    return box(double(double(e)))

def offset_momentum(ref, bodies, px=0.0, py=0.0, pz=0.0):
    for body in bodies:
        v: Vector = cast(Body, body).v
        m = cast(Body, body).mass
        px -= v.x * double(m)
        py -= v.y * double(m)
        pz -= double(v.z) * double(m)
    m = box(double(double(cast(Body, ref).mass)))
    v = cast(Body, ref).v
    v.x = double(px) / double(m)
...
def bench_nbody(loops, reference: str, iterations: int):
    ...
    for _ in range_it:
        double(report_energy(cast(CheckedList[Body], SYSTEM), PAIRS))
