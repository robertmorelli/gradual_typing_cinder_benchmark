...
class Vector:
    ...
    def __init__(self, x: double, y: double, z):
        self.x = box(double(x))
        self.y = box(double(y))
        self.z = z
...
def combinations(l: CheckedList[Body]) -> list[tuple[Body, Body]]:
    ...
    for x in range(len(l) - 1):
        ...
        for y in ls:
            result.append((l[x], y))
    return result
BODIES = CheckedDict[str, Body]({'sun': Body(Vector(0.0, 0.0, 0.0), Vector(0.0, 0.0, 0.0), box(double(double(SOLAR_MASS)))), 'jupiter': Body(Vector(4.841431442464721, -1.1603200440274284, box(double(-0.10362204447112311))), Vector(0.001660076642744037 * double(DAYS_PER_YEAR), 0.007699011184197404 * double(DAYS_PER_YEAR), box(double(-6.90460016972063e-05 * double(DAYS_PER_YEAR)))), box(double(0.0009547919384243266 * double(SOLAR_MASS)))), 'saturn': Body(Vector(8.34336671824458, 4.124798564124305, box(double(-0.4035234171143214))), Vector(-0.002767425107268624 * double(DAYS_PER_YEAR), 0.004998528012349172 * double(DAYS_PER_YEAR), box(double(2.3041729757376393e-05 * double(DAYS_PER_YEAR)))), box(double(0.0002858859806661308 * double(SOLAR_MASS)))), 'uranus': Body(Vector(12.894369562139131, -15.111151401698631, box(double(-0.22330757889265573))), Vector(0.002964601375647616 * double(DAYS_PER_YEAR), 0.0023784717395948095 * double(DAYS_PER_YEAR), box(double(-2.9658956854023756e-05 * double(DAYS_PER_YEAR)))), box(double(4.366244043351563e-05 * double(SOLAR_MASS)))), 'neptune': Body(Vector(15.379697114850917, -25.919314609987964, 0.17925877295037118), Vector(0.0026806777249038932 * double(DAYS_PER_YEAR), 0.001628241700382423 * double(DAYS_PER_YEAR), box(double(-9.515922545197159e-05 * double(DAYS_PER_YEAR)))), box(double(5.1513890204661145e-05 * double(SOLAR_MASS))))})
...
def offset_momentum(ref, bodies, px: double=0.0, py=0.0, pz: double=0.0):
    for body in bodies:
        v = cast(Body, body).v
        m: double = double(cast(Body, body).mass)
        px -= double(v.x) * m
        py -= double(v.y) * m
        pz -= double(v.z) * m
    m = double(cast(Body, ref).mass)
    v = cast(Body, ref).v
    v.x = box(double(px / m))
    v.y = box(double(double(py) / m))
    v.z = box(double(pz / m))
