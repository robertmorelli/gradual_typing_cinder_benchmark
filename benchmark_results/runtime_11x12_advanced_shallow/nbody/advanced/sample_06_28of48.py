...
def advance(dt, n, bodies: CheckedList[Body]=SYSTEM, pairs=PAIRS):
    for i in range(n):
        for b1, b2 in pairs:
            ...
            mag = double(dt) * (dx * dx + double(dy) * double(dy) + dz * dz) ** double(-1.5)
...
def report_energy(bodies: CheckedList[Body]=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS, e=0.0) -> double:
    b2: Body
    for b1, b2 in pairs:
        pos1 = cast(Vector, cast(Body, b1).pos)
    ...
    for body in bodies:
        v = cast(Body, body).v
        e += double(cast(Body, body).mass) * (double(v.x) * double(v.x) + v.y * v.y + v.z * v.z) / 2.0
    return double(e)
