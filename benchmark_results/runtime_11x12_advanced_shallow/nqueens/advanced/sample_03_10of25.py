...
def create_array(start, end: int64, step) -> Array[int64]:
    ...
    size = box(int64(int64((static_abs(end - int64(start)) - 1) / static_abs(int64(step)) + 1)))
    a: Array[int64] = Array[int64](size)
...
def permutations(pool: Array[int64], r: int64=-1):
    n = clen(pool)
    if r == -1:
        r = n
    rb = box(r)
    indices = create_array(0, n, 1)
    cycles: Array[int64] = create_array(box(int64(n)), n - r, box(int64(-1)))
