...
class GVector(object):
    ...
    def linear_combination(self, other, l1: float, l2: float):
        v = GVector(self.x * l1 + cast(GVector, other).x * l2, self.y * l1 + cast(GVector, other).y * l2, self.z * l1 + cast(GVector, other).z * l2)
        return cast(GVector, v)

class Spline(object):

    def __init__(self, points: CheckedList[GVector], degree, knots: CheckedList[int]):
        if knots == None:
...
class Chaosgame(object):
    ...
    def __init__(self, splines, thickness: float, w: int, h: int, n):
