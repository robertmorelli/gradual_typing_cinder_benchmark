...
class Strength(object):
    ...
    @classmethod
    def stronger(cls, s1: Strength, s2: Strength):
        return s1.strength < s2.strength

    @classmethod
    def weaker(cls, s1, s2: Strength) -> bool:
        return cast(Strength, s1).strength > s2.strength

    @classmethod
    def weakest_of(cls, s1, s2):
        if cls.weaker(cast(Strength, s1), cast(Strength, s2)):
            return cast(Strength, cast(Strength, s1))
        return cast(Strength, cast(Strength, s2))
...
class Planner(object):
    ...
    def add_propagate(self, c: Constraint, mark) -> bool:
...
def change(v: Variable, new_value: int):
    planner = get_planner()
