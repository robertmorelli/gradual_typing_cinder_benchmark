...
class Constraint(object):
    ...
    def satisfy(self, mark):
        ...
        cast(Variable, out).mark = int64(mark)
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if cast(Variable, self.my_output).mark != int64(mark) and stronger(self.strength, cast(Variable, self.my_output).walk_strength):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if cast(Variable, self.v1).mark == int64(mark):
            if self.v2.mark != int64(mark) and stronger(self.strength, self.v2.walk_strength):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if self.v2.mark == int64(mark):
            if cast(Variable, self.v1).mark != int64(mark) and stronger(self.strength, cast(Variable, self.v1).walk_strength):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
...
class ScaleConstraint(BinaryConstraint):
    ...
    def recalculate(self):
        ...
        out.stay = box(cbool(cbool(cast(Variable, ihn).stay) and cbool(cast(Variable, self.scale).stay) and cbool(self.offset.stay)))
        if cbool(out.stay):
            self.execute()
