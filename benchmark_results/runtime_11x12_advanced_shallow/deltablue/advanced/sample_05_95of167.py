...
def weaker(s1, s2) -> cbool:
    return int64(cast(Strength, s1).strength) > int64(cast(Strength, s2).strength)
...
class UrnaryConstraint(Constraint):
    ...
    def __init__(self, v, strength: Strength):
    ...
    def choose_method(self, mark):
        if int64(self.my_output.mark) != int64(mark) and cbool(stronger(self.strength, cast(Strength, self.my_output.walk_strength))):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def __init__(self, v1, v2: Variable, strength):
        Constraint.__init__(self, strength)
        self.v1: Variable = v1
        self.v2 = v2
    ...
    def choose_method(self, mark):
        if int64(self.v1.mark) == int64(mark):
            if int64(cast(Variable, self.v2).mark) != int64(mark) and cbool(stronger(self.strength, cast(Strength, cast(Variable, self.v2).walk_strength))):
        ...
        if weaker(cast(Strength, self.v1.walk_strength), cast(Strength, cast(Variable, self.v2).walk_strength)):
            if cbool(stronger(self.strength, cast(Strength, self.v1.walk_strength))):
                ...
                self.direction = Direction.NONE
        elif cbool(stronger(self.strength, cast(Strength, cast(Variable, self.v2).walk_strength))):
            self.direction = Direction.FORWARD
        else:
            self.direction = Direction.BACKWARD
    ...
    def inputs_known(self, mark):
        ...
        return box(cbool(int64(cast(Variable, i).mark) == int64(mark) or cast(Variable, i).stay or cbool(cast(Constraint | None, cast(Variable, i).determined_by) is None)))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src, scale: Variable, offset, dest, strength: Strength):
        self.direction = Direction.NONE
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            cast(Variable, self.v2).value = self.v1.value * self.scale.value + self.offset.value
        else:
            self.v1.value = (cast(Variable, self.v2).value - self.offset.value) / self.scale.value

    def recalculate(self):
        ihn: Variable = self.input()
        out = self.output()
        cast(Variable, out).walk_strength = weakest_of(self.strength, cast(Strength, ihn.walk_strength))
        cast(Variable, out).stay = ihn.stay and self.scale.stay and self.offset.stay
        if cast(Variable, out).stay:
            self.execute()
...
class Planner(object):
    ...
    def incremental_add(self, constraint: Constraint):
        ...
        overridden: Constraint | None = constraint.satisfy(box(int64(mark)))
