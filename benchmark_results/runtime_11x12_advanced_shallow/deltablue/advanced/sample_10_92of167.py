...
def weakest_of(s1: Strength, s2) -> Strength:
    return s1 if s1.strength > cast(Strength, s2).strength else s2
...
class Constraint(object):
    ...
    def satisfy(self, mark: int64):
        ...
        out = self.output()
        overridden = cast(Constraint | None, cast(Variable, out).determined_by)
        if overridden is not None:
            cast(Constraint, overridden).mark_unsatisfied()
        cast(Variable, out).determined_by = self
        if not cast(Planner, planner).add_propagate(self, mark):
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        ...
        if cast(Variable, self.v2).mark == int64(mark):
            if self.v1.mark != int64(mark) and stronger(cast(Strength, self.strength), cast(Strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
    ...
    def recalculate(self):
        ...
        out.walk_strength = weakest_of(cast(Strength, self.strength), cast(Strength, cast(Variable, ihn).walk_strength))
    ...
    def inputs_known(self, mark):
        ...
        return box(cbool(i.mark == int64(mark) or i.stay or cbool(i.determined_by is None)))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            cast(Variable, self.v2).value = box(int64(int64(self.v1.value) * int64(self.scale.value) + int64(cast(Variable, self.offset).value)))
        else:
            self.v1.value = box(int64((int64(cast(Variable, self.v2).value) - int64(cast(Variable, self.offset).value)) / int64(self.scale.value)))

    def recalculate(self):
        ihn = self.input()
        out = self.output()
        cast(Variable, out).walk_strength = weakest_of(cast(Strength, self.strength), cast(Strength, cast(Variable, ihn).walk_strength))
        cast(Variable, out).stay = cast(Variable, ihn).stay and self.scale.stay and cast(Variable, self.offset).stay
