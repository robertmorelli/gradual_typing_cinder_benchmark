...
@inline
def weaker(s1: Strength, s2):
    return box(cbool(s1.strength > cast(Strength, s2).strength))
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if self.my_output.mark != int64(mark) and stronger(self.strength, self.my_output.walk_strength):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if self.v1.mark == int64(mark):
            if cast(Variable, self.v2).mark != int64(mark) and stronger(self.strength, cast(Variable, self.v2).walk_strength):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if cast(Variable, self.v2).mark == int64(mark):
            if self.v1.mark != int64(mark) and stronger(self.strength, self.v1.walk_strength):
                self.direction = Direction.BACKWARD
            else:
...
class Planner(object):
    ...
    def incremental_add(self, constraint: Constraint) -> None:
        ...
        overridden: Constraint | None = cast(Constraint | None, constraint.satisfy(box(int64(mark))))
    ...
    def make_plan(self, sources: CheckedList[UrnaryConstraint]):
        ...
        while clen(todo):
            c = todo.pop(0)
            if cast(Variable, c.output()).mark != mark and c.inputs_known(box(int64(mark))):
