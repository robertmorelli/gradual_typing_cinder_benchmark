...
class BinaryConstraint(Constraint):
    ...
    def __init__(self, v1, v2: Variable, strength: Strength):
        Constraint.__init__(self, strength)
        self.v1 = v1
        self.v2 = v2
    ...
    def choose_method(self, mark) -> None:
        ...
        if int64(cast(Variable, self.v2).mark) == int64(mark):
            if int64(cast(Variable, self.v1).mark) != int64(mark) and stronger(self.strength, cast(Variable, self.v1).walk_strength):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        if cbool(weaker(cast(Variable, self.v1).walk_strength, cast(Variable, self.v2).walk_strength)):
            if stronger(self.strength, cast(Variable, self.v1).walk_strength):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src, scale: Variable, offset: Variable, dest, strength: Strength):
        self.direction = Direction.NONE
