...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if self.my_output.mark != int64(mark) and stronger(cast(Strength, self.strength), self.my_output.walk_strength):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if cast(Variable, self.v1).mark == int64(mark):
            if cast(Variable, self.v2).mark != int64(mark) and stronger(cast(Strength, self.strength), cast(Variable, self.v2).walk_strength):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if cast(Variable, self.v2).mark == int64(mark):
            if cast(Variable, self.v1).mark != int64(mark) and stronger(cast(Strength, self.strength), cast(Variable, self.v1).walk_strength):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        if weaker(cast(Variable, self.v1).walk_strength, cast(Variable, self.v2).walk_strength):
            if stronger(cast(Strength, self.strength), cast(Variable, self.v1).walk_strength):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        elif stronger(cast(Strength, self.strength), cast(Variable, self.v2).walk_strength):
            self.direction = Direction.FORWARD
        else:
            self.direction = Direction.BACKWARD
    ...
    def recalculate(self):
        ...
        cast(Variable, out).stay = ihn.stay
...
class ScaleConstraint(BinaryConstraint):
    ...
    def recalculate(self):
        ...
        out = self.output()
        cast(Variable, out).walk_strength = weakest_of(cast(Strength, self.strength), ihn.walk_strength)
        cast(Variable, out).stay = ihn.stay and self.scale.stay and cast(Variable, self.offset).stay
        if cast(Variable, out).stay:
            self.execute()
...
def change(v, new_value: int64) -> None:
    ...
    i: int64 = 0
    while i < 10:
        cast(Variable, v).value = box(int64(new_value))
        plan.execute()
        i = i + 1
