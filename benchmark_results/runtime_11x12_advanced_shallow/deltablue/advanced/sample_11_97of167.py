...
def stronger(s1, s2: Strength) -> cbool:
    return int64(cast(Strength, s1).strength) < int64(s2.strength)
...
@inline
def weakest_of(s1: Strength, s2):
    return cast(Strength, s1 if int64(s1.strength) > int64(cast(Strength, s2).strength) else s2)
...
class Constraint(object):
    ...
    def satisfy(self, mark: int64):
        ...
        out = self.output()
        overridden: Constraint | None = cast(Constraint | None, cast(Variable, out).determined_by)
        if overridden is not None:
            overridden.mark_unsatisfied()
        cast(Variable, out).determined_by = self
        if not cbool(planner.add_propagate(self, mark)):
            print('Cycle encountered')
        cast(Variable, out).mark = box(int64(mark))
...
class BinaryConstraint(Constraint):
    ...
    def recalculate(self):
        ...
        cast(Variable, out).stay = box(cbool(cbool(ihn.stay)))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def recalculate(self):
        ...
        out = self.output()
        cast(Variable, out).walk_strength = cast(Strength, weakest_of(cast(Strength, self.strength), cast(Strength, ihn.walk_strength)))
        cast(Variable, out).stay = box(cbool(cbool(ihn.stay) and cbool(cast(Variable, self.scale).stay) and cbool(cast(Variable, self.offset).stay)))
        if cbool(cast(Variable, out).stay):
            self.execute()
...
class Planner(object):
    ...
    def incremental_add(self, constraint):
        mark = self.new_mark()
        overridden = cast(Constraint, constraint).satisfy(int64(mark))
    ...
    def extract_plan_from_constraints(self, constraints) -> Plan:
        ...
        for c in cast(CheckedList[UrnaryConstraint], constraints):
            if c.is_input() and c.is_satisfied():
                sources.append(c)
        return _repro_make_plan_arg0(self.make_plan, sources)
...
def chain_test(n):
    ...
    edits.append(edit)
    plan = _repro_extract_plan_from_constraints_arg0(cast(Planner, planner).extract_plan_from_constraints, edits)
    i = 0
    while i < 100:
