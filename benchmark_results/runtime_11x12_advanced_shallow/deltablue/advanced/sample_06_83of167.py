...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if cast(Variable, self.my_output).mark != int64(mark) and cbool(stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.my_output).walk_strength))):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def __init__(self, v1: Variable, v2, strength):
        Constraint.__init__(self, strength)
        self.v1 = v1
        self.v2: Variable = v2
    ...
    def choose_method(self, mark):
        if cast(Variable, self.v1).mark == int64(mark):
            if self.v2.mark != int64(mark) and cbool(stronger(cast(Strength, self.strength), cast(Strength, self.v2.walk_strength))):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if self.v2.mark == int64(mark):
            if cast(Variable, self.v1).mark != int64(mark) and cbool(stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v1).walk_strength))):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src: Variable, scale, offset, dest: Variable, strength):
        self.direction = Direction.NONE
    ...
    def execute(self) -> None:
        if self.direction == Direction.FORWARD:
            self.v2.value = cast(Variable, self.v1).value * self.scale.value + self.offset.value
        else:
            cast(Variable, self.v1).value = (self.v2.value - self.offset.value) / self.scale.value

    def recalculate(self):
        ihn: Variable = cast(Variable, self.input())
        out = self.output()
        cast(Variable, out).walk_strength = weakest_of(cast(Strength, self.strength), cast(Strength, ihn.walk_strength))
...
class Planner(object):
    ...
    def incremental_add(self, constraint) -> None:
        ...
        while overridden is not None:
            overridden = cast(Constraint, overridden).satisfy(mark)
...
def change(v: Variable, new_value: int64) -> None:
    ...
    edits: CheckedList[UrnaryConstraint] = []
    edits.append(edit)
    plan = _repro_extract_plan_from_constraints_arg0_2(planner.extract_plan_from_constraints, edits)
    i = box(int64(0))
    while int64(i) < 10:
        v.value = new_value
        plan.execute()
