...
@inline
def weaker(s1, s2: Strength) -> cbool:
    return cast(Strength, s1).strength > s2.strength

@inline
def weakest_of(s1, s2):
    return cast(Strength, s1 if cast(Strength, s1).strength > cast(Strength, s2).strength else s2)
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if cast(Variable, self.my_output).mark != int64(mark) and cbool(stronger(cast(Strength, self.strength), cast(Variable, self.my_output).walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if self.v1.mark == int64(mark):
            if cast(Variable, self.v2).mark != int64(mark) and cbool(stronger(cast(Strength, self.strength), cast(Variable, self.v2).walk_strength)):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if cast(Variable, self.v2).mark == int64(mark):
            if self.v1.mark != int64(mark) and cbool(stronger(cast(Strength, self.strength), self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
    ...
    def recalculate(self):
        ihn = self.input()
        out = self.output()
        cast(Variable, out).walk_strength = cast(Strength, weakest_of(cast(Strength, self.strength), cast(Variable, ihn).walk_strength))
    ...
    def inputs_known(self, mark):
        ...
        return box(cbool(i.mark == int64(mark) or i.stay or cbool(cast(Constraint | None, i.determined_by) is None)))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src: Variable, scale, offset, dest, strength):
