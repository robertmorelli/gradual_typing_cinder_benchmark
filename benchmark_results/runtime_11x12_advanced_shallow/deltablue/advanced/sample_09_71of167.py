...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark) -> None:
        ...
        if int64(cast(Variable, self.v2).mark) == int64(mark):
            if int64(cast(Variable, self.v1).mark) != int64(mark) and cbool(stronger(cast(Strength, self.strength), cast(Variable, self.v1).walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        if cbool(weaker(cast(Variable, self.v1).walk_strength, cast(Variable, self.v2).walk_strength)):
    ...
    def inputs_known(self, mark: int64) -> cbool:
        i = self.input()
        return int64(cast(Variable, i).mark) == mark or cbool(cast(Variable, i).stay) or cbool(cast(Variable, i).determined_by is None)
...
class EqualityConstraint(BinaryConstraint):
    ...
    def execute(self) -> None:
        self.output().value = box(int64(int64(cast(Variable, self.input()).value)))
