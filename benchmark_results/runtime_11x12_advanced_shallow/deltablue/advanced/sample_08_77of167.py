...
class Constraint(object):
    ...
    def satisfy(self, mark: int64) -> Constraint | None:
        ...
        overridden: Constraint | None = cast(Constraint | None, out.determined_by)
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        ...
        if int64(self.v2.mark) == int64(mark):
            if int64(cast(Variable, self.v1).mark) != int64(mark) and cbool(stronger(self.strength, cast(Strength, cast(Variable, self.v1).walk_strength))):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        if weaker(cast(Strength, cast(Variable, self.v1).walk_strength), cast(Strength, self.v2.walk_strength)):
            if cbool(stronger(self.strength, cast(Strength, cast(Variable, self.v1).walk_strength))):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
...
class Planner(object):
    ...
    def incremental_remove(self, constraint) -> None:
        out: Variable = cast(Constraint, constraint).output()
