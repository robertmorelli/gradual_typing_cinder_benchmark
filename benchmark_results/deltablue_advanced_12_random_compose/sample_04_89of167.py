...
class Strength:
    ...
    def next_weaker(self) -> Strength:
        return cast(Strength, STRENGTHS[box(self.strength)])
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64):
        if int64(self.my_output.mark) != mark and stronger(self.strength, cast(Strength, self.my_output.walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64):
        if int64(self.v1.mark) == mark:
            if int64(cast(Variable, self.v2).mark) != mark and stronger(self.strength, cast(Strength, cast(Variable, self.v2).walk_strength)):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if int64(cast(Variable, self.v2).mark) == mark:
            if int64(self.v1.mark) != mark and stronger(self.strength, cast(Strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        if cbool(weaker(cast(Strength, self.v1.walk_strength), cast(Strength, cast(Variable, self.v2).walk_strength))):
            if stronger(self.strength, cast(Strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        elif stronger(self.strength, cast(Strength, cast(Variable, self.v2).walk_strength)):
            self.direction = Direction.FORWARD
        else:
            self.direction = Direction.BACKWARD
    ...
    def recalculate(self):
        ihn = self.input()
        out: Variable = cast(Variable, self.output())
        out.walk_strength = cast(Strength, weakest_of(self.strength, cast(Strength, cast(Variable, ihn).walk_strength)))
    ...
    def inputs_known(self, mark) -> cbool:
        ...
        return int64(i.mark) == int64(mark) or i.stay or cbool(i.determined_by is None)
...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            cast(Variable, self.v2).value = self.v1.value * cast(Variable, self.scale).value + self.offset.value
        else:
            self.v1.value = (cast(Variable, self.v2).value - self.offset.value) / cast(Variable, self.scale).value

    def recalculate(self):
        ihn = self.input()
        out: Variable = cast(Variable, self.output())
        out.walk_strength = cast(Strength, weakest_of(self.strength, cast(Strength, cast(Variable, ihn).walk_strength)))
        out.stay = cast(Variable, ihn).stay and cast(Variable, self.scale).stay and self.offset.stay
        if out.stay:
            self.execute()
...
class Planner(object):
    ...
    def incremental_add(self, constraint):
        ...
        overridden = cast(Constraint | None, cast(Constraint, constraint).satisfy(int64(mark)))
        while overridden is not None:
            overridden = cast(Constraint, overridden).satisfy(int64(mark))
