...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if int64(cast(Variable, self.v1).mark) == mark:
            if int64(self.v2.mark) != mark and stronger(cast(Strength, self.strength), cast(Strength, self.v2.walk_strength)):
        ...
        if weaker(cast(Strength, cast(Variable, self.v1).walk_strength), cast(Strength, self.v2.walk_strength)):
    ...
    def inputs_known(self, mark):
        ...
        return box(cbool(int64(i.mark) == int64(mark) or cbool(i.stay) or cbool(i.determined_by is None)))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def recalculate(self) -> None:
        ...
        out.stay = box(cbool(cbool(cast(Variable, ihn).stay) and cbool(self.scale.stay) and cbool(cast(Variable, self.offset).stay)))
        if cbool(out.stay):
            self.execute()
