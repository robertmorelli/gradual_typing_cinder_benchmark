...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src, scale: Variable, offset, dest: Variable, strength):
        self.direction = Direction.NONE
    ...
    def recalculate(self):
        ...
        cast(Variable, out).stay = cast(Variable, ihn).stay and self.scale.stay and self.offset.stay
...
class Planner(object):
    ...
    def make_plan(self, sources: CheckedList[UrnaryConstraint]) -> Plan:
        ...
        while clen(todo):
            c = todo.pop(0)
            if c.output().mark != int64(mark) and cbool(c.inputs_known(mark)):
