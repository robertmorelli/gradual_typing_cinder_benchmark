...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        ...
        if cast(Variable, self.v2).mark == int64(mark):
            if self.v1.mark != int64(mark) and cbool(stronger(self.strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
    ...
    def inputs_known(self, mark) -> cbool:
        i = self.input()
        return cast(Variable, i).mark == int64(mark) or cbool(cast(Variable, i).stay) or cbool(cast(Variable, i).determined_by is None)
...
class ScaleConstraint(BinaryConstraint):
    ...
    def recalculate(self):
        ...
        cast(Variable, out).stay = box(cbool(cbool(cast(Variable, ihn).stay) and cbool(self.scale.stay) and cbool(self.offset.stay)))
...
class Planner(object):
    ...
    def incremental_add(self, constraint) -> None:
        mark = self.new_mark()
        overridden: Constraint | None = cast(Constraint, constraint).satisfy(int64(mark))
