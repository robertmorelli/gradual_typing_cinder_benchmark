...
class BinaryConstraint(Constraint):
    ...
    def inputs_known(self, mark) -> cbool:
        ...
        return int64(i.mark) == int64(mark) or i.stay or cbool(cast(Constraint | None, i.determined_by) is None)
...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            cast(Variable, self.v2).value = box(int64(int64(self.v1.value) * int64(cast(Variable, self.scale).value) + int64(cast(Variable, self.offset).value)))
        else:
            self.v1.value = box(int64((int64(cast(Variable, self.v2).value) - int64(cast(Variable, self.offset).value)) / int64(cast(Variable, self.scale).value)))
