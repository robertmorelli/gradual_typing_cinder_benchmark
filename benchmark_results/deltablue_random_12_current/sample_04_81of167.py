...
def weaker(s1, s2: Strength):
    return box(cbool(cast(Strength, s1).strength > s2.strength))
...
class Constraint(object):
    ...
    def satisfy(self, mark):
        ...
        if not cast(Planner, planner).add_propagate(self, int64(mark)):
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if cast(Variable, self.v1).mark == int64(mark):
            if cast(Variable, self.v2).mark != int64(mark) and cbool(stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v2).walk_strength))):
...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            cast(Variable, self.v2).value = cast(Variable, self.v1).value * self.scale.value + cast(Variable, self.offset).value
        else:
            cast(Variable, self.v1).value = (cast(Variable, self.v2).value - cast(Variable, self.offset).value) / self.scale.value
    ...
    def recalculate(self) -> None:
        ...
        cast(Variable, out).stay = box(cbool(cbool(ihn.stay) and cbool(self.scale.stay) and cbool(cast(Variable, self.offset).stay)))
