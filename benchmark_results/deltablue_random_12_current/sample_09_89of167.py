...
def weakest_of(s1, s2: Strength):
    return cast(Strength, s1 if int64(cast(Strength, s1).strength) > int64(s2.strength) else s2)
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark) -> None:
        if cast(Variable, self.my_output).mark != int64(mark) and cbool(stronger(self.strength, cast(Variable, self.my_output).walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def recalculate(self):
        ...
        out = self.output()
        cast(Variable, out).walk_strength = cast(Strength, weakest_of(self.strength, ihn.walk_strength))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src: Variable, scale, offset, dest: Variable, strength: Strength):
