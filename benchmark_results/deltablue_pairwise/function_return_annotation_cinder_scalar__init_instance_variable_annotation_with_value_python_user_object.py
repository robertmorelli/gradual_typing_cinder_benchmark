...
def stronger(s1: Strength, s2: Strength):
    return box(cbool(s1.strength < s2.strength))
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if cast(Variable, self.my_output).mark != mark and cbool(stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.my_output).walk_strength))):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if cast(Variable, self.v1).mark == mark:
            if cast(Variable, self.v2).mark != mark and cbool(stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v2).walk_strength))):
        ...
        if cast(Variable, self.v2).mark == mark:
            if cast(Variable, self.v1).mark != mark and cbool(stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v1).walk_strength))):
                self.direction = cast(Direction, Direction.BACKWARD)
            else:
