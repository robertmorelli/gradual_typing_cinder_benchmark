...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark) -> None:
        if cast(Variable, self.v1).mark == int64(mark):
            if cast(Variable, self.v2).mark != int64(mark) and stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v2).walk_strength)):
...
class Planner(object):
    ...
    def incremental_add(self, constraint: Constraint) -> None:
        mark = self.new_mark()
        overridden = constraint.satisfy(box(int64(mark)))
        while overridden is not None:
            overridden = overridden.satisfy(box(int64(mark)))
    ...
    def make_plan(self, sources: CheckedList[UrnaryConstraint]) -> Plan:
        ...
        while clen(todo):
            c = todo.pop(0)
            if c.output().mark != mark and c.inputs_known(box(int64(mark))):
