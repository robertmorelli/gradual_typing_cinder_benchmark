...
class Constraint(object):
    ...
    def __init__(self, strength: Strength):
...
class UrnaryConstraint(Constraint):
    ...
    def __init__(self, v: Variable, strength: Strength):
    ...
    def choose_method(self, mark: int64) -> None:
        if cast(Variable, self.my_output).mark != mark and stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.my_output).walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
    ...
    def recalculate(self) -> None:
        ...
        cast(Variable, self.my_output).stay = not self.is_input()
...
class BinaryConstraint(Constraint):
    ...
    def __init__(self, v1: Variable, v2: Variable, strength: Strength):
        Constraint.__init__(self, strength)
        self.v1 = v1
        self.v2 = v2
    ...
    def choose_method(self, mark: int64) -> None:
        if cast(Variable, self.v1).mark == mark:
            if cast(Variable, self.v2).mark != mark and stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v2).walk_strength)):
                self.direction = cast(Direction, Direction.FORWARD)
            else:
                self.direction = cast(Direction, Direction.BACKWARD)
        if cast(Variable, self.v2).mark == mark:
            if cast(Variable, self.v1).mark != mark and stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v1).walk_strength)):
                self.direction = cast(Direction, Direction.BACKWARD)
            else:
                self.direction = cast(Direction, Direction.NONE)
        if weaker(cast(Strength, cast(Variable, self.v1).walk_strength), cast(Strength, cast(Variable, self.v2).walk_strength)):
            if stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v1).walk_strength)):
                self.direction = cast(Direction, Direction.BACKWARD)
            else:
                self.direction = cast(Direction, Direction.NONE)
        elif stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v2).walk_strength)):
            self.direction = cast(Direction, Direction.FORWARD)
        else:
            self.direction = cast(Direction, Direction.BACKWARD)
    ...
    def is_satisfied(self) -> cbool:
        if cast(Direction, self.direction) != Direction.NONE:
            return True
        return False
    ...
    def input(self) -> Variable:
        return cast(Variable, self.v1) if cast(Direction, self.direction) == Direction.FORWARD else cast(Variable, self.v2)

    def output(self) -> Variable:
        return cast(Variable, self.v2) if cast(Direction, self.direction) == Direction.FORWARD else cast(Variable, self.v1)
    ...
    def inputs_known(self, mark: int64) -> cbool:
        i = self.input()
        return i.mark == mark or i.stay or cbool(cast(Constraint | None, i.determined_by) is None)
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src: Variable, scale: Variable, offset: Variable, dest: Variable, strength: Strength):
        self.direction = Direction.NONE
        self.scale = scale
        self.offset = offset
    ...
    def mark_inputs(self, mark: int64) -> None:
        ...
        cast(Variable, self.scale).mark = mark
        cast(Variable, self.offset).mark = mark

    def execute(self) -> None:
        if cast(Direction, self.direction) == Direction.FORWARD:
            cast(Variable, self.v2).value = cast(Variable, self.v1).value * cast(Variable, self.scale).value + cast(Variable, self.offset).value
        else:
            cast(Variable, self.v1).value = (cast(Variable, self.v2).value - cast(Variable, self.offset).value) / cast(Variable, self.scale).value

    def recalculate(self) -> None:
        ihn = self.input()
        out = self.output()
        out.walk_strength = cast(Strength, weakest_of(cast(Strength, self.strength), cast(Strength, ihn.walk_strength)))
        out.stay = ihn.stay and cast(Variable, self.scale).stay and cast(Variable, self.offset).stay
        if out.stay:
            self.execute()
...
class Variable(object):
    ...
    def __init__(self, name: str, initial_value: int64=0):
        self.name: str = name
...
class Planner(object):
    ...
    def remove_propagate_from(self, out: Variable) -> CheckedList[Constraint]:
        out.determined_by = cast(Constraint | None, None)
        out.walk_strength = cast(Strength, WEAKEST)
        out.stay = True
