...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if int64(self.my_output.mark) != mark and stronger(self.strength, self.my_output.walk_strength):
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if int64(self.v1.mark) == mark:
            if int64(self.v2.mark) != mark and stronger(self.strength, self.v2.walk_strength):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if int64(self.v2.mark) == mark:
            if int64(self.v1.mark) != mark and stronger(self.strength, self.v1.walk_strength):
                self.direction = Direction.BACKWARD
            else:
...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self) -> None:
        if self.direction == Direction.FORWARD:
            self.v2.value = box(int64(int64(self.v1.value) * int64(self.scale.value) + int64(self.offset.value)))
        else:
            self.v1.value = box(int64((int64(self.v2.value) - int64(self.offset.value)) / int64(self.scale.value)))

    def recalculate(self) -> None:
        ihn = self.input()
        out = self.output()
        out.walk_strength = weakest_of(self.strength, ihn.walk_strength)
        out.stay = box(cbool(cbool(ihn.stay) and cbool(self.scale.stay) and cbool(self.offset.stay)))
        if cbool(out.stay):
            self.execute()
