...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if int64(cast(Variable, self.my_output).mark) != mark and stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.my_output).walk_strength)):
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if int64(cast(Variable, self.v1).mark) == mark:
            if int64(cast(Variable, self.v2).mark) != mark and stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v2).walk_strength)):
                self.direction = cast(Direction, Direction.FORWARD)
            else:
                self.direction = cast(Direction, Direction.BACKWARD)
        if int64(cast(Variable, self.v2).mark) == mark:
            if int64(cast(Variable, self.v1).mark) != mark and stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v1).walk_strength)):
                self.direction = cast(Direction, Direction.BACKWARD)
            else:
    ...
    def inputs_known(self, mark: int64) -> cbool:
        i = self.input()
        return int64(i.mark) == mark or cbool(i.stay) or cbool(cast(Constraint | None, i.determined_by) is None)
...
class EqualityConstraint(BinaryConstraint):
    ...
    def execute(self) -> None:
        self.output().value = box(int64(int64(self.input().value)))
...
class Planner(object):
    ...
    def make_plan(self, sources: CheckedList[UrnaryConstraint]) -> Plan:
        ...
        while clen(todo):
            c = todo.pop(0)
            if int64(c.output().mark) != mark and c.inputs_known(mark):
                plan.add_constraint(c)
                c.output().mark = box(int64(mark))
