...
class Constraint(object):
    ...
    def satisfy(self, mark: int64) -> Constraint | None:
        ...
        out = self.output()
        overridden = cast(Constraint, out.determined_by)
        if overridden is not None:
            overridden.mark_unsatisfied()
        out.determined_by = cast(Constraint, self)
...
class BinaryConstraint(Constraint):
    ...
    def inputs_known(self, mark: int64):
        i = self.input()
        return box(cbool(i.mark == mark or i.stay or cbool(cast(Constraint, i.determined_by) is None)))
...
class Variable(object):
    ...
    def remove_constraint(self, constraint: Constraint) -> None:
        ...
        if cast(Constraint, self.determined_by) == constraint:
            self.determined_by = cast(Constraint, None)
...
class Planner(object):
    ...
    def remove_propagate_from(self, out: Variable) -> CheckedList[Constraint]:
        out.determined_by = cast(Constraint, None)
        ...
        while len(todo):
            ...
            determining = cast(Constraint, v.determined_by)
            for c in cs:
