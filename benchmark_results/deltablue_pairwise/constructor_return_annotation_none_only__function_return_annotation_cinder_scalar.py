...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if self.my_output.mark != mark and cbool(stronger(self.strength, self.my_output.walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if self.v1.mark == mark:
            if self.v2.mark != mark and cbool(stronger(self.strength, self.v2.walk_strength)):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if self.v2.mark == mark:
            if self.v1.mark != mark and cbool(stronger(self.strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
