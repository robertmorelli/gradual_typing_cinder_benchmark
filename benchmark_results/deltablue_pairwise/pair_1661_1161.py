...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if self.my_output.mark != mark and stronger(self.strength, cast(Strength, self.my_output.walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if self.v1.mark == mark:
            if self.v2.mark != mark and stronger(self.strength, cast(Strength, self.v2.walk_strength)):
    ...
    def recalculate(self) -> None:
        ihn = self.input()
        out = self.output()
        out.walk_strength = cast(Strength, weakest_of(self.strength, cast(Strength, ihn.walk_strength)))
        out.stay = ihn.stay
        if out.stay:
            self.execute()
