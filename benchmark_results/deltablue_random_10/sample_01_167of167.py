...
class Strength:
    ...
    def next_weaker(self):
        return cast(CheckedList[Strength], STRENGTHS)[self.strength]
...
class BinaryConstraint(Constraint):
    ...
    def recalculate(self):
        ...
        cast(Variable, out).stay = box(cbool(cast(Variable, ihn).stay))
    ...
    def inputs_known(self, mark):
        i = self.input()
        return box(int64(cast(Variable, i).mark) == int64(mark) or cbool(cast(Variable, i).stay) or cbool(cast(Constraint | None, cast(Variable, i).determined_by) is None))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def recalculate(self):
        ...
        out = self.output()
        cast(Variable, out).walk_strength = cast(Strength, weakest_of(cast(Strength, self.strength), cast(Strength, cast(Variable, ihn).walk_strength)))
        cast(Variable, out).stay = box(cbool(cast(Variable, ihn).stay) and cbool(cast(Variable, self.scale).stay) and cbool(cast(Variable, self.offset).stay))
        if cbool(cast(Variable, out).stay):
            self.execute()
...
class Plan(object):
    ...
    def add_constraint(self, c):
        cast(CheckedList[Constraint], self.v).append(cast(Constraint, c))
...
def chain_test(n):
    ...
    edits.append(edit)
    plan = cast(Plan, cast(Planner, planner).extract_plan_from_constraints(cast(CheckedList[UrnaryConstraint], edits)))
    i = box(int64(0))
    while int64(i) < 100:
        first.value = i
        plan.execute()
