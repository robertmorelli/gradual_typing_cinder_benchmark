...
class BinaryConstraint(Constraint):
    ...
    def inputs_known(self, mark):
        ...
        return box(cast(Variable, i).mark == int64(mark) or cbool(cast(Variable, i).stay) or cbool(cast(Variable, i).determined_by is None))
...
class Planner(object):
    ...
    def make_plan(self, sources: CheckedList[UrnaryConstraint]) -> Plan:
        ...
        while clen(todo):
            c = todo.pop(0)
            if c.output().mark != mark and cbool(c.inputs_known(box(mark))):
