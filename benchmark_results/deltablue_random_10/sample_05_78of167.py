...
class Constraint(object):
    ...
    def satisfy(self, mark: int64):
        ...
        if not cbool(cast(Planner, planner).add_propagate(self, box(mark))):
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if int64(cast(Variable, self.my_output).mark) != int64(mark) and stronger(self.strength, cast(Variable, self.my_output).walk_strength):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def inputs_known(self, mark: int64):
        ...
        return box(int64(i.mark) == mark or cbool(i.stay) or cbool(i.determined_by is None))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def recalculate(self) -> None:
        ...
        cast(Variable, out).stay = box(cbool(cast(Variable, ihn).stay) and cbool(cast(Variable, self.scale).stay) and cbool(self.offset.stay))
...
class Variable(object):
    ...
    def remove_constraint(self, constraint: Constraint) -> None:
        cast(CheckedList[Constraint], self.constraints).remove(constraint)
...
class Planner(object):
    ...
    def incremental_add(self, constraint) -> None:
        ...
        overridden: Constraint | None = cast(Constraint | None, constraint.satisfy(mark))
...
def change(v, new_value) -> None:
    ...
    edits: CheckedList[UrnaryConstraint] = []
    edits.append(edit)
    plan = cast(Planner, planner).extract_plan_from_constraints(cast(CheckedList[UrnaryConstraint], edits))
    i = 0
    while int64(i) < 10:
