...
@inline
def weaker(s1, s2: Strength):
    return box(int64(s1.strength) > int64(s2.strength))

@inline
def weakest_of(s1, s2):
    return cast(Strength, cast(Strength, s1) if int64(s1.strength) > int64(s2.strength) else cast(Strength, s2))
...
class Constraint(object):
    ...
    def satisfy(self, mark: int64) -> Constraint | None:
        ...
        if not cast(Planner, planner).add_propagate(self, box(mark)):
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if cast(Variable, self.my_output).mark != int64(mark) and stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.my_output).walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
    ...
    def recalculate(self):
        ...
        cast(Variable, self.my_output).stay = box(not cbool(self.is_input()))
...
class BinaryConstraint(Constraint):
    ...
    def input(self):
        return cast(Variable, cast(Variable, self.v1) if self.direction == Direction.FORWARD else self.v2)

    def output(self):
        return cast(Variable, self.v2 if self.direction == Direction.FORWARD else cast(Variable, self.v1))

    def recalculate(self):
        ihn: Variable = cast(Variable, self.input())
        out = self.output()
        cast(Variable, out).walk_strength = cast(Strength, weakest_of(cast(Strength, self.strength), cast(Strength, ihn.walk_strength)))
        cast(Variable, out).stay = box(cbool(ihn.stay))
    ...
    def inputs_known(self, mark) -> cbool:
        i: Variable = cast(Variable, self.input())
        return i.mark == int64(mark) or cbool(i.stay) or cbool(i.determined_by is None)
...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            self.v2.value = box(int64(cast(Variable, self.v1).value) * int64(self.scale.value) + int64(self.offset.value))
        else:
            cast(Variable, self.v1).value = box((int64(self.v2.value) - int64(self.offset.value)) / int64(self.scale.value))

    def recalculate(self):
        ihn = self.input()
        out: Variable = cast(Variable, self.output())
        out.walk_strength = cast(Strength, weakest_of(cast(Strength, self.strength), cast(Strength, cast(Variable, ihn).walk_strength)))
        out.stay = box(cbool(cast(Variable, ihn).stay) and cbool(self.scale.stay) and cbool(self.offset.stay))
        if cbool(out.stay):
            self.execute()
...
class EqualityConstraint(BinaryConstraint):
    ...
    def execute(self):
        cast(Variable, self.output()).value = box(int64(cast(Variable, self.input()).value))
...
def projection_test(n) -> None:
    ...
    while int64(i) < int64(n) - 1:
        if int64(dests[int64(i)].value) != int64(i) * 5 + 1000:
            print('Projection 3 failed')
        i = box(int64(int64(i) + 1))
    change(offset, 2000)
    i = box(int64(0))
    while int64(i) < int64(n) - 1:
        if int64(dests[int64(i)].value) != int64(i) * 5 + 2000:
