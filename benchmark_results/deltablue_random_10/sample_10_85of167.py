...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if int64(self.my_output.mark) != int64(mark) and stronger(self.strength, cast(Strength, self.my_output.walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if int64(self.v1.mark) == int64(mark):
            if int64(self.v2.mark) != int64(mark) and stronger(self.strength, cast(Strength, self.v2.walk_strength)):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if int64(self.v2.mark) == int64(mark):
            if int64(self.v1.mark) != int64(mark) and stronger(self.strength, cast(Strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
    ...
    def inputs_known(self, mark: int64):
        i = self.input()
        return box(int64(cast(Variable, i).mark) == mark or cast(Variable, i).stay or cbool(cast(Variable, i).determined_by is None))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def mark_inputs(self, mark):
        ...
        self.offset.mark = box(int64(mark))

    def execute(self):
        if self.direction == Direction.FORWARD:
            self.v2.value = box(int64(self.v1.value) * int64(cast(Variable, self.scale).value) + int64(self.offset.value))
        else:
            self.v1.value = box((int64(self.v2.value) - int64(self.offset.value)) / int64(cast(Variable, self.scale).value))

    def recalculate(self):
        ihn: Variable = cast(Variable, self.input())
        out = self.output()
        cast(Variable, out).walk_strength = weakest_of(self.strength, cast(Strength, ihn.walk_strength))
        cast(Variable, out).stay = ihn.stay and cast(Variable, self.scale).stay and self.offset.stay
...
class Variable(object):
    ...
    def add_constraint(self, constraint: Constraint):
        cast(CheckedList[Constraint], self.constraints).append(constraint)
    ...
    def make_plan(self, sources: CheckedList[UrnaryConstraint]):
        ...
        while clen(todo):
            ...
            if int64(cast(Variable, c.output()).mark) != mark and cbool(c.inputs_known(mark)):
                ...
                cast(Variable, c.output()).mark = box(mark)
