...
class Constraint(object):
    ...
    def satisfy(self, mark) -> Constraint | None:
        ...
        if not cbool(cast(Planner, planner).add_propagate(self, int64(mark))):
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        ...
        if cbool(weaker(self.v1.walk_strength, self.v2.walk_strength)):
            if cbool(stronger(self.strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        elif cbool(stronger(self.strength, self.v2.walk_strength)):
            self.direction = Direction.FORWARD
        else:
            self.direction = Direction.BACKWARD
    ...
    def mark_inputs(self, mark):
        cast(Variable, self.input()).mark = box(int64(mark))
    ...
    def recalculate(self):
        ihn = self.input()
        out = self.output()
        cast(Variable, out).walk_strength = cast(Strength, weakest_of(self.strength, cast(Variable, ihn).walk_strength))
    ...
    def inputs_known(self, mark: int64):
        ...
        return box(int64(i.mark) == mark or i.stay or cbool(cast(Constraint | None, i.determined_by) is None))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src, scale, offset, dest: Variable, strength):
        self.direction: Direction = Direction.NONE
    ...
    def mark_inputs(self, mark):
        ...
        self.scale.mark = box(int64(mark))
        cast(Variable, self.offset).mark = box(int64(mark))

    def execute(self):
        if self.direction == Direction.FORWARD:
            self.v2.value = box(int64(self.v1.value) * int64(self.scale.value) + int64(cast(Variable, self.offset).value))
        else:
            self.v1.value = box((int64(self.v2.value) - int64(cast(Variable, self.offset).value)) / int64(self.scale.value))

    def recalculate(self):
        ihn = self.input()
        out: Variable = cast(Variable, self.output())
        out.walk_strength = cast(Strength, weakest_of(self.strength, cast(Variable, ihn).walk_strength))
        out.stay = cast(Variable, ihn).stay and self.scale.stay and cast(Variable, self.offset).stay
        if out.stay:
            self.execute()
...
class Planner(object):
    ...
    def incremental_add(self, constraint: Constraint):
        ...
        overridden: Constraint | None = constraint.satisfy(box(mark))
    ...
    def make_plan(self, sources) -> Plan:
        ...
        while clen(todo):
            c = todo.pop(0)
            if int64(cast(Variable, c.output()).mark) != int64(mark) and cbool(c.inputs_known(int64(mark))):
