...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            cast(Variable, self.v2).value = box(int64(self.v1.value) * int64(self.scale.value) + int64(self.offset.value))
        else:
            self.v1.value = box((int64(cast(Variable, self.v2).value) - int64(self.offset.value)) / int64(self.scale.value))
...
class Planner(object):
    ...
    def incremental_add(self, constraint):
        ...
        overridden = constraint.satisfy(box(mark))
        while overridden is not None:
            overridden = cast(Constraint | None, cast(Constraint, overridden).satisfy(box(mark)))
