...
class GVector(object):
    ...
    def __init__(self, x, y: float, z: float) -> None:
        self.x = x
        self.y = y
        self.z = z
    ...
    def dist(self, other) -> float:
        return math.sqrt((self.x - cast(GVector, other).x) ** 2 + (self.y - cast(GVector, other).y) ** 2 + (self.z - cast(GVector, other).z) ** 2)
    ...
    def __sub__(self, other: GVector):
        return cast(GVector, self + other * -1)

    def __mul__(self, other) -> GVector:
        v = GVector(self.x * other, self.y * other, self.z * other)
        return v
    __rmul__ = __mul__

    def linear_combination(self, other, l1: float, l2):
        v = GVector(self.x * l1 + cast(GVector, other).x * l2, self.y * l1 + cast(GVector, other).y * l2, self.z * l1 + cast(GVector, other).z * l2)
        return cast(GVector, v)

class Spline(object):

    def __init__(self, points, degree: int, knots: CheckedList[int]):
        if knots == None:
    ...
    def __call__(self, u) -> GVector:
        ...
        for ik in range(1, self.degree + 1):
            for ii in range(I - self.degree + ik + 1, I + 2):
                ...
                d[index] = cast(GVector, d[index].linear_combination(d[index + 1], co1, co2))
        return d[0]

class Chaosgame(object):

    def __init__(self, splines: List[Spline], thickness: float, w: int, h: int, n: int):
        ...
        for _ in range(n):
            for i in range(5000):
                point = cast(GVector, self.transform_point(point))
    ...
    def transform_point(self, point):
        x = (cast(GVector, point).x - self.minx) / self.width
        y = (cast(GVector, point).y - self.miny) / self.height
        ...
        return cast(GVector, basepoint)
if __name__ == '__main__':
    splines = [Spline(CheckedList[GVector]([GVector(1.59735, 3.30446, 0.0), GVector(1.57581, 4.12326, 0.0), GVector(1.31321, 5.28835, 0.0), GVector(1.6189, 5.32991, 0.0), GVector(2.88994, 5.5027, 0.0), GVector(2.37306, 4.38183, 0.0), GVector(1.662, 4.36028, 0.0)]), 3, [0, 0, 0, 1, 1, 1, 2, 2, 2]), Spline(CheckedList[GVector]([GVector(2.8045, 4.01735, 0.0), GVector(2.5505, 3.52523, 0.0), GVector(1.97901, 2.62036, 0.0), GVector(1.97901, 2.62036, 0.0)]), 3, [0, 0, 0, 1, 1, 1]), Spline(CheckedList[GVector]([GVector(2.00167, 4.01132, 0.0), GVector(2.33504, 3.31283, 0.0), GVector(2.3668, 3.23346, 0.0), GVector(2.3668, 3.23346, 0.0)]), 3, [0, 0, 0, 1, 1, 1])]
    startTime = time.time()
