...
class GVector(object):
    ...
    def __init__(self, x: float, y, z) -> None:
        self.x = x
        self.y: float = y
        self.z = z
    ...
    def __add__(self, other) -> GVector:
        if not isinstance(cast(GVector, other), GVector):
            raise ValueError("Can't add GVector to " + str(type(cast(GVector, other))))
        v = GVector(self.x + cast(GVector, other).x, self.y + cast(GVector, other).y, self.z + cast(GVector, other).z)
        return v
    ...
    def linear_combination(self, other: GVector, l1: float, l2) -> GVector:
...
class Spline(object):
    ...
    def __call__(self, u):
        ...
        if u == dom[0]:
            return cast(GVector, self.points[0])
        if u == dom[1]:
            return cast(GVector, self.points[-1])
        I = None
        ...
        return cast(GVector, d[0])

class Chaosgame(object):

    def __init__(self, splines, thickness: float, w: int, h, n: int):
