...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if int64(self.v1.mark) == int64(mark):
            if int64(cast(Variable, self.v2).mark) != int64(mark) and cbool(stronger(self.strength, cast(Variable, self.v2).walk_strength)):
    ...
    def output(self):
        return cast(Variable, cast(Variable, self.v2) if cast(Direction, self.direction) == Direction.FORWARD else self.v1)

    def recalculate(self):
        ihn: Variable = cast(Variable, self.input())
        out = cast(Variable, cast(Variable, self.output()))
        out.walk_strength = cast(Strength, weakest_of(self.strength, ihn.walk_strength))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src: Variable, scale, offset, dest, strength: Strength):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            cast(Variable, self.v2).value = box(int64(int64(self.v1.value) * int64(cast(Variable, self.scale).value) + int64(self.offset.value)))
        else:
            self.v1.value = box(int64((int64(cast(Variable, self.v2).value) - int64(self.offset.value)) / int64(cast(Variable, self.scale).value)))

    def recalculate(self):
        ihn: Variable = cast(Variable, self.input())
        out = cast(Variable, cast(Variable, self.output()))
        out.walk_strength = cast(Strength, weakest_of(self.strength, ihn.walk_strength))
        out.stay = box(cbool(cbool(ihn.stay) and cbool(cast(Variable, self.scale).stay) and cbool(self.offset.stay)))
        if cbool(out.stay):
            self.execute()
...
class Planner(object):
    ...
    def make_plan(self, sources: CheckedList[UrnaryConstraint]) -> Plan:
        ...
        while clen(todo):
            c = todo.pop(0)
            if int64(cast(Variable, c.output()).mark) != mark and c.inputs_known(mark):
