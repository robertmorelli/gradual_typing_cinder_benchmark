...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if int64(cast(Variable, self.my_output).mark) != int64(mark) and cbool(stronger(self.strength, cast(Strength, cast(Variable, self.my_output).walk_strength))):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def __init__(self, v1, v2, strength: Strength):
        Constraint.__init__(self, strength)
        self.v1: Variable = v1
        self.v2: Variable = v2
    ...
    def choose_method(self, mark):
        if int64(self.v1.mark) == int64(mark):
            if int64(self.v2.mark) != int64(mark) and cbool(stronger(self.strength, cast(Strength, self.v2.walk_strength))):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if int64(self.v2.mark) == int64(mark):
            if int64(self.v1.mark) != int64(mark) and cbool(stronger(self.strength, cast(Strength, self.v1.walk_strength))):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        if cbool(weaker(cast(Strength, self.v1.walk_strength), cast(Strength, self.v2.walk_strength))):
            if cbool(stronger(self.strength, cast(Strength, self.v1.walk_strength))):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        elif cbool(stronger(self.strength, cast(Strength, self.v2.walk_strength))):
            self.direction = Direction.FORWARD
        else:
            self.direction = Direction.BACKWARD
    ...
    def input(self):
        return cast(Variable, self.v1 if cast(Direction, self.direction) == Direction.FORWARD else self.v2)

    def output(self) -> Variable:
        return self.v2 if cast(Direction, self.direction) == Direction.FORWARD else self.v1
    ...
    def inputs_known(self, mark: int64):
        ...
        return box(cbool(int64(i.mark) == mark or i.stay or cbool(i.determined_by is None)))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def __init__(self, src, scale, offset: Variable, dest: Variable, strength: Strength):
        self.direction = Direction.NONE
    ...
    def execute(self):
        if cast(Direction, self.direction) == Direction.FORWARD:
            self.v2.value = self.v1.value * self.scale.value + cast(Variable, self.offset).value
        else:
            self.v1.value = (self.v2.value - cast(Variable, self.offset).value) / self.scale.value
...
class Variable(object):
    ...
    def __init__(self, name, initial_value: int64=0) -> None:
        self.name = name
