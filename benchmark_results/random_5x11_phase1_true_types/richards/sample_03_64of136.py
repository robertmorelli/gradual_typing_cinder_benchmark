...
class Packet(object):
    ...
    def __init__(self, l, i: int64, k: int64) -> None:
...
class TaskState(object):
    ...
    def isWaitingWithPacket(self):
        return box(cbool(cbool(self.packet_pending) and cbool(self.task_waiting) and (not self.task_holding)))
tracing = False
layout = 0
...
class Task(TaskState):
    ...
    def __init__(self, i: int64, p: int64, w, initialState, r):
        wa = cast(TaskWorkArea, taskWorkArea)
        self.link = wa.taskList
        self.ident: int64 = i
        self.priority: int64 = p
        self.input: Optional[Packet] = w
        self.packet_pending = box(cbool(initialState.isPacketPending()))
        self.task_waiting = box(cbool(cbool(initialState.isTaskWaiting())))
        self.task_holding = cbool(initialState.isTaskHolding())
        self.handle = r
        wa.taskList = self
        cast(List[Task], wa.taskTab)[i] = self
    ...
    def qpkt(self, pkt: Packet) -> Task:
        t: Task = cast(Task, Task.findtcb(box(int64(self)), int64(pkt.ident)))
...
class DeviceTask(Task):
    ...
    def __init__(self, i: int64, p: int64, w, s, r):
...
class WorkTask(Task):
    ...
    def fn(self, pkt, r):
        ...
        pkt.ident = box(int64(int64(dest)))
        pkt.datum = 0
        i = 0
        while i < BUFSIZE:
            x: int64 = w.count + 1
