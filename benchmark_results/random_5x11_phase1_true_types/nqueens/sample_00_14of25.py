...
def create_array(start: int64, end: int64, step):
    """
    ...
    c = box(int64(start))
    i: int64 = 0
    if (end - start) * int64(step) <= 0:
        return Array[int64](0)
    size: int64 = int64((static_abs(end - start) - 1) / static_abs(int64(step)) + 1)
    a = Array[int64](box(size))
    while i < size:
        a[i] = int64(c)
...
def permutations(pool, r: int64=-1):
    n = clen(pool)
    if r == -1:
        r = n
    rb = box(r)
    indices = create_array(0, n, box(int64(1)))
    cycles: Array[int64] = create_array(n, n - r, box(int64(-1)))
    per = Array[int64](rb)
    idx: int64 = 0
    while idx < r:
...
def solve(queen_count) -> Iterator[Array[int64]]:
    ...
    static_cols = create_array(0, int64(queen_count), box(int64(1)))
