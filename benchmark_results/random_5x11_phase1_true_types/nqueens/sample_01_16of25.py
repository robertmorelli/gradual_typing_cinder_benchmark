...
def create_array(start, end: int64, step):
    """
    ...
    c: int64 = int64(start)
    i: int64 = 0
    if (end - int64(start)) * int64(step) <= 0:
        return Array[int64](0)
    size = box(int64(int64((int64(static_abs(box(int64(end - int64(start))))) - 1) / int64(static_abs(box(int64(step)))) + 1)))
    a = Array[int64](box(int64(size)))
    while i < int64(size):
        a[i] = c
        c = c + int64(step)
        i = i + 1
...
def permutations(pool: Array[int64], r=-1):
    n = clen(pool)
    if int64(r) == -1:
        r = box(n)
    rb = box(int64(r))
    indices: Array[int64] = create_array(box(int64(0)), n, box(int64(1)))
    cycles = create_array(box(int64(n)), n - int64(r), box(int64(-1)))
...
def solve(queen_count):
    ...
    static_cols: Array[int64] = create_array(box(int64(0)), int64(queen_count), box(int64(1)))
    for vec in permutations(static_cols):
        if queen_count == len(set((vec[i] + i for i in cols))) == len(set((vec[i] - i for i in cols))):
            yield vec

def bench_n_queens(queen_count) -> List[Array[int64]]:
    """
