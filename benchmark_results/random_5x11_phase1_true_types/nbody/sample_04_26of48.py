...
class Vector:
    ...
    def __init__(self, x, y, z: double):
        self.x = box(double(double(x)))
        self.y = box(double(double(y)))
        self.z = box(double(z))
...
class Body:
    ...
    def __init__(self, pos: Vector, v, mass: double):
        self.pos = pos
        self.v: Vector = v
...
def combinations(l: CheckedList[Body]) -> list[tuple[Body, Body]]:
    ...
    for x in range(len(l) - 1):
        ...
        for y in ls:
            result.append((l[x], y))
    return result
BODIES = CheckedDict[str, Body]({'sun': Body(Vector(box(double(0.0)), box(double(0.0)), 0.0), Vector(box(double(0.0)), box(double(0.0)), 0.0), double(SOLAR_MASS)), 'jupiter': Body(Vector(box(double(4.841431442464721)), box(double(-1.1603200440274284)), -0.10362204447112311), Vector(box(double(0.001660076642744037 * double(DAYS_PER_YEAR))), box(double(0.007699011184197404 * double(DAYS_PER_YEAR))), -6.90460016972063e-05 * double(DAYS_PER_YEAR)), 0.0009547919384243266 * double(SOLAR_MASS)), 'saturn': Body(Vector(box(double(8.34336671824458)), box(double(4.124798564124305)), -0.4035234171143214), Vector(box(double(-0.002767425107268624 * double(DAYS_PER_YEAR))), box(double(0.004998528012349172 * double(DAYS_PER_YEAR))), 2.3041729757376393e-05 * double(DAYS_PER_YEAR)), 0.0002858859806661308 * double(SOLAR_MASS)), 'uranus': Body(Vector(box(double(12.894369562139131)), box(double(-15.111151401698631)), -0.22330757889265573), Vector(box(double(0.002964601375647616 * double(DAYS_PER_YEAR))), box(double(0.0023784717395948095 * double(DAYS_PER_YEAR))), -2.9658956854023756e-05 * double(DAYS_PER_YEAR)), 4.366244043351563e-05 * double(SOLAR_MASS)), 'neptune': Body(Vector(box(double(15.379697114850917)), box(double(-25.919314609987964)), 0.17925877295037118), Vector(box(double(0.0026806777249038932 * double(DAYS_PER_YEAR))), box(double(0.001628241700382423 * double(DAYS_PER_YEAR))), -9.515922545197159e-05 * double(DAYS_PER_YEAR)), 5.1513890204661145e-05 * double(SOLAR_MASS))})
SYSTEM: CheckedList[Body] = CheckedList[Body](BODIES.values())
PAIRS: list[tuple[Body, Body]] = combinations(SYSTEM)

def advance(dt, n, bodies: CheckedList[Body]=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS):
    for i in range(n):
        b2: Body
        for b1, b2 in pairs:
...
def report_energy(bodies=SYSTEM, pairs: list[tuple[Body, Body]]=PAIRS, e: double=0.0) -> double:
    for b1, b2 in pairs:
        pos1 = cast(Vector, b1.pos)
        pos2 = cast(Vector, b2.pos)
    ...
    for body in cast(CheckedList[Body], bodies):
        v = body.v
        e += body.mass * (double(v.x) * double(v.x) + double(v.y) * double(v.y) + double(v.z) * double(v.z)) / 2.0
    return e
