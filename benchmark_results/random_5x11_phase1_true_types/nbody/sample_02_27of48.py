...
class Body:
    ...
    def __init__(self, pos, v: Vector, mass: double):
        self.pos = pos
        self.v = v
...
def combinations(l) -> list[tuple[Body, Body]]:
    ...
    result = []
    y: Body
    for x in range(len(cast(CheckedList[Body], l)) - 1):
        ls: CheckedList[Body] = cast(CheckedList[Body], l)[x + 1:]
        for y in ls:
            result.append((cast(CheckedList[Body], l)[x], y))
    return result
...
def offset_momentum(ref: Body, bodies, px=0.0, py: double=0.0, pz=0.0):
    for body in cast(CheckedList[Body], bodies):
        v = cast(Vector, cast(Vector, body.v))
