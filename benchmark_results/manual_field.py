...
def stronger(s1: Strength, s2: Strength) -> cbool:
    return cast(Strength, cast(Strength, s1.strength)) < cast(Strength, cast(Strength, s2.strength))
...
def weakest_of(s1: Strength, s2: Strength) -> Strength:
    return s1 if cast(Strength, cast(Strength, s1.strength)) > cast(Strength, cast(Strength, s2.strength)) else s2
...
class Strength:
    ...
    def next_weaker(self) -> Strength:
        return STRENGTHS[cast(Strength, cast(Strength, self.strength))]
...
class Constraint(object):
    ...
    def satisfy(self, mark: int64) -> Constraint | None:
        ...
        if not self.is_satisfied():
            if cast(Strength, cast(Strength, self.strength)) == REQUIRED:
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if self.my_output.mark != mark and stronger(cast(Strength, cast(Strength, self.strength)), self.my_output.walk_strength):
            self.satisfied = True
        else:
            self.satisfied = False
    ...
    def recalculate(self) -> None:
        self.my_output.walk_strength = cast(Strength, cast(Strength, self.strength))
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: int64) -> None:
        if self.v1.mark == mark:
            if self.v2.mark != mark and stronger(cast(Strength, cast(Strength, self.strength)), self.v2.walk_strength):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if self.v2.mark == mark:
            if self.v1.mark != mark and stronger(cast(Strength, cast(Strength, self.strength)), self.v1.walk_strength):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        if weaker(self.v1.walk_strength, self.v2.walk_strength):
            if stronger(cast(Strength, cast(Strength, self.strength)), self.v1.walk_strength):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        elif stronger(cast(Strength, cast(Strength, self.strength)), self.v2.walk_strength):
            self.direction = Direction.FORWARD
        else:
            self.direction = Direction.BACKWARD
    ...
    def recalculate(self) -> None:
        ihn = self.input()
        out = self.output()
        out.walk_strength = weakest_of(cast(Strength, cast(Strength, self.strength)), ihn.walk_strength)
        out.stay = ihn.stay
        if out.stay:
            self.execute()
    ...
    def incremental_remove(self, constraint: Constraint) -> None:
        ...
        while repeat:
            for u in unsatisfied:
                if cast(Strength, cast(Strength, u.strength)) == strength:
