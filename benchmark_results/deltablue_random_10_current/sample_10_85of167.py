...
class BinaryConstraint(Constraint):
    ...
    def inputs_known(self, mark: int64):
        i = self.input()
        return box(cbool(int64(cast(Variable, i).mark) == mark or cast(Variable, i).stay or cbool(cast(Variable, i).determined_by is None)))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            self.v2.value = box(int64(int64(self.v1.value) * int64(cast(Variable, self.scale).value) + int64(self.offset.value)))
        else:
            self.v1.value = box(int64((int64(self.v2.value) - int64(self.offset.value)) / int64(cast(Variable, self.scale).value)))
