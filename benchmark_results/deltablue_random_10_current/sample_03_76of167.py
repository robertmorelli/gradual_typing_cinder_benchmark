...
class ScaleConstraint(BinaryConstraint):
    ...
    def execute(self):
        if self.direction == Direction.FORWARD:
            self.v2.value = box(int64(int64(cast(Variable, self.v1).value) * int64(self.scale.value) + int64(self.offset.value)))
        else:
            cast(Variable, self.v1).value = box(int64((int64(self.v2.value) - int64(self.offset.value)) / int64(self.scale.value)))
    ...
    def recalculate(self):
        ...
        out.stay = box(cbool(cbool(cast(Variable, ihn).stay) and cbool(self.scale.stay) and cbool(self.offset.stay)))
        if cbool(out.stay):
            self.execute()
