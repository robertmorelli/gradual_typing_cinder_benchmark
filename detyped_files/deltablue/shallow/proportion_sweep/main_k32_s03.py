...
class Strength(object):
    ...
    @classmethod
    def stronger(cls, _s1, _s2):
        s1: Strength = cast(Strength, _s1)
        s2: Strength = cast(Strength, _s2)
        return s1.strength < s2.strength

    @classmethod
    def weaker(cls, _s1, _s2):
        s1: Strength = cast(Strength, _s1)
        s2: Strength = cast(Strength, _s2)
        return s1.strength > s2.strength

    @classmethod
    def weakest_of(cls, _s1, _s2):
        s1: Strength = cast(Strength, _s1)
        s2: Strength = cast(Strength, _s2)
        if cls.weaker(cast(Strength, s1), cast(Strength, s2)):
            return s1
        return s2

    @classmethod
    def strongest(cls, s1, s2):
        if cls.stronger(cast(Strength, s1), cast(Strength, s2)):
            return s1
        return s2
...
class Constraint(object):
    ...
    def satisfy(self, mark):
        ...
        if not planner.add_propagate(cast(Constraint, self), mark):
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if self.my_output.mark != mark and Strength.stronger(cast(Strength, self.strength), cast(Strength, self.my_output.walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
    ...
    def remove_from_graph(self):
        if self.my_output is not None:
            self.my_output.remove_constraint(cast(EditConstraint, self))
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if self.v1.mark == mark:
            if self.v2.mark != mark and Strength.stronger(cast(Strength, self.strength), cast(Strength, self.v2.walk_strength)):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if self.v2.mark == mark:
            if self.v1.mark != mark and Strength.stronger(cast(Strength, self.strength), cast(Strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        if Strength.weaker(cast(Strength, self.v1.walk_strength), cast(Strength, self.v2.walk_strength)):
            if Strength.stronger(cast(Strength, self.strength), cast(Strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
        elif Strength.stronger(cast(Strength, self.strength), cast(Strength, self.v2.walk_strength)):
            self.direction = Direction.FORWARD
        else:
            self.direction = Direction.BACKWARD
    ...
    def recalculate(self):
        ...
        out = cast(Variable, self.output())
        out.walk_strength = cast(Strength, Strength.weakest_of(cast(Strength, self.strength), cast(Strength, ihn.walk_strength)))
        out.stay = ihn.stay
        if out.stay:
            self.execute()
    ...
    def remove_from_graph(self):
        if self.v1 is not None:
            self.v1.remove_constraint(cast(EditConstraint, self))
        if self.v2 is not None:
            self.v2.remove_constraint(cast(EditConstraint, self))
        self.direction = Direction.NONE
...
class ScaleConstraint(BinaryConstraint):
    ...
    def remove_from_graph(self):
        ...
        if self.scale is not None:
            self.scale.remove_constraint(cast(EditConstraint, self))
        if self.offset is not None:
            self.offset.remove_constraint(cast(EditConstraint, self))
...
class Variable(object):
    ...
    def remove_constraint(self, _constraint):
        constraint: EditConstraint = cast(EditConstraint, _constraint)
...
class Planner(object):
    ...
    def incremental_remove(self, _constraint):
        ...
        unsatisfied = cast(OrderedCollection, self.remove_propagate_from(cast(Variable, out)))
    ...
    def make_plan(self, _sources):
        sources: OrderedCollection = cast(OrderedCollection, _sources)
        mark = self.new_mark()
        plan = Plan()
        todo = sources
        while len(todo):
            ...
            if cast(Variable, c.output()).mark != mark and c.inputs_known(mark):
                ...
                self.add_constraints_consuming_to(cast(Variable, c.output()), cast(OrderedCollection, todo))
        return plan

    def extract_plan_from_constraints(self, _constraints):
        constraints: OrderedCollection = cast(OrderedCollection, _constraints)
        ...
        while i < x:
            ...
            if c.is_input() and c.is_satisfied():
                sources.append(c)
            i = i + 1
        return cast(Plan, self.make_plan(cast(OrderedCollection, sources)))

    def add_propagate(self, _c, mark):
        ...
        while len(todo):
            ...
            if cast(Variable, d.output()).mark == mark:
                ...
                return False
            d.recalculate()
            self.add_constraints_consuming_to(cast(Variable, d.output()), cast(OrderedCollection, todo))
        return True
    ...
    def add_constraints_consuming_to(self, _v, _coll):
        ...
        coll: OrderedCollection = cast(OrderedCollection, _coll)
        determining = v.determined_by
        cc = v.constraints
        x = len(cc)
        i = 0
...
def chain_test(n):
    ...
    edits.append(edit)
    plan = cast(Plan, planner.extract_plan_from_constraints(cast(OrderedCollection, edits)))
    i = 0
    while i < 100:
        first.value = i
        plan.execute()
...
def projection_test(n):
    ...
    change(cast(Variable, dst), 1050)
    if src.value != 5:
        print('Projection 2 failed')
    change(cast(Variable, scale), 5)
    i = 0
    while i < n - 1:
        if dests[i].value != i * 5 + 1000:
            print('Projection 3 failed')
        i = i + 1
    change(cast(Variable, offset), 2000)
    i = 0
    while i < n - 1:
