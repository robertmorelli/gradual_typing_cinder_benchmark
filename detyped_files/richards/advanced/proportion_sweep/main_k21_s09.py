...
class Packet(object):
    ...
    def append_to(self, _lst):
        ...
        if lst is None:
            ...
            p = cast(Packet, lst)
            next = cast(Optional[Packet], p.link)
...
class Task(TaskState):
    ...
    def runTask(self):
        if TaskState.isWaitingWithPacket(self):
            msg = cast(Optional[Packet], self.input)
            if msg is not None:
                self.input = msg.link
                if self.input is None:
            ...
            msg = None
        return cast(Optional[Task], self.fn(cast(Optional[Packet], msg), cast(TaskRec, self.handle)))
    ...
    def release(self, _i):
        i: int64 = int64(_i)
        t = cast(Task, cast(Task, Task.findtcb(self, box(int64(i)))))
    ...
    def qpkt(self, _pkt):
        ...
        t = cast(Task, cast(Task, Task.findtcb(self, box(int64(pkt.ident)))))
...
class HandlerTask(Task):
    ...
    def fn(self, _pkt, _r):
        ...
        work = cast(Optional[Packet], h.work_in)
        if work is None:
            return cast(Task, self.waitTask())
        ...
        dev = cast(Optional[Packet], h.device_in)
        if dev is None:
            return cast(Task, self.waitTask())
...
class WorkTask(Task):
    ...
    def fn(self, _pkt, _r):
        ...
        if w.destination == int64(I_HANDLERA):
            dest: int64 = int64(int64(I_HANDLERB))
        else:
...
def schedule():
    t = cast(Optional[Task], taskWorkArea.taskList)
    while t is not None:
        if tracing:
...
class Richards(object):
    ...
    def run(self, iterations):
        for i in range(iterations):
            ...
            wkq = cast(Optional[Packet], Packet(None, 0, int64(K_WORK)))
