...
class Packet(object):
    ...
    def append_to(self, _lst):
        lst: Packet | None = cast(Packet | None, _lst)
        self.link = None
        if lst is None:
            return self
        else:
...
class HandlerTaskRec(TaskRec):
    ...
    def workInAdd(self, _p):
        ...
        self.work_in = cast(Packet, p.append_to(cast(Packet | None, self.work_in)))
        return self.work_in
...
class Task(TaskState):
    ...
    def fn(self, _pkt, _r):
        pkt: Packet | None = cast(Packet | None, _pkt)
    ...
    def runTask(self):
        if self.isWaitingWithPacket():
            ...
            msg = None
        return cast(Task | None, self.fn(cast(Packet | None, msg), cast(TaskRec, self.handle)))
...
class DeviceTask(Task):
    ...
    def fn(self, _pkt, _r):
        ...
        if pkt is None:
            ...
            if tracing:
                trace(pkt.datum)
            return cast(Task, cast(Task | None, self.hold()))
...
class HandlerTask(Task):
    ...
    def fn(self, _pkt, _r):
        ...
        if pkt is not None:
            if pkt.kind == K_WORK:
                cast(Packet | None, h.workInAdd(cast(Packet, pkt)))
            else:
                cast(Packet | None, h.deviceInAdd(cast(Packet, pkt)))
        work = h.work_in
        if work is None:
            return cast(Task, self.waitTask())
        count = work.datum
...
class IdleTask(Task):
    ...
    def fn(self, _pkt, _r):
        pkt: Packet | None = cast(Packet | None, _pkt)
        r: TaskRec = cast(TaskRec, _r)
        i = cast(IdleTaskRec, cast(IdleTaskRec, r))
        i.count -= 1
        if i.count == 0:
            return cast(Task | None, self.hold())
        elif i.control & 1 == 0:
            i.control //= 2
            return cast(Task, self.release(I_DEVA))
        else:
            i.control = i.control // 2 ^ 53256
            return cast(Task, self.release(I_DEVB))
A: Final[int] = 65
...
def schedule():
    t = cast(Task | None, taskWorkArea.taskList)
    while t is not None:
        if tracing:
            print('tcb =', t.ident)
...
class Richards(object):
    ...
    def run(self, iterations):
        for i in range(iterations):
            ...
            WorkTask(I_WORK, 1000, wkq, cast(TaskState, TaskState().waitingWithPacket()), WorkerTaskRec())
            wkq = Packet(None, I_DEVA, K_DEV)
            wkq = Packet(wkq, I_DEVA, K_DEV)
            wkq = Packet(wkq, I_DEVA, K_DEV)
            HandlerTask(I_HANDLERA, 2000, wkq, cast(TaskState, TaskState().waitingWithPacket()), HandlerTaskRec())
            wkq = Packet(None, I_DEVB, K_DEV)
            wkq = Packet(wkq, I_DEVB, K_DEV)
            wkq = Packet(wkq, I_DEVB, K_DEV)
            HandlerTask(I_HANDLERB, 3000, wkq, cast(TaskState, TaskState().waitingWithPacket()), HandlerTaskRec())
            wkq = None
