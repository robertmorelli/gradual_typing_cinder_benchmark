...
def combinations(_l):
    l: List[T] = cast(List[T], _l)
    'Pure-Python implementation of itertools.combinations(l, 2).'
    result = cast(List[Tuple[T, T]], [])
...
def advance(dt, n, _bodies=SYSTEM, _pairs=PAIRS):
    bodies: List[Body] = cast(List[Body], _bodies)
    pairs: List[Tuple[Body, Body]] = cast(List[Tuple[Body, Body]], _pairs)
    for i in range(n):
...
def report_energy(_bodies=SYSTEM, _pairs=PAIRS, e=0.0):
    bodies: List[Body] = cast(List[Body], _bodies)
    pairs: List[Tuple[Body, Body]] = cast(List[Tuple[Body, Body]], _pairs)
    for (((x1, y1, z1), v1, m1), ((x2, y2, z2), v2, m2)) in pairs:
        dx = x1 - x2
...
def offset_momentum(_ref, _bodies=SYSTEM, px=0.0, py=0.0, pz=0.0):
...
def bench_nbody(loops, reference, iterations):
    offset_momentum(cast(Body, BODIES[reference]))
