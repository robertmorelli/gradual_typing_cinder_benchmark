...
def weakest_of(s1, s2):
    return cast(Strength, s1 if int64(s1.strength) > int64(s2.strength) else s2)
...
class Constraint(object):
    ...
    def satisfy(self, mark):
        ...
        if not cbool(planner.add_propagate(self, box(mark))):
            print('Cycle encountered')
        out.mark = box(int64(mark))
        return cast(Constraint | Unknown, overridden)
...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if int64(cast(Variable, self.my_output).mark) != mark and cbool(stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.my_output).walk_strength))):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark):
        if int64(cast(Variable, self.v1).mark) == mark:
            if int64(cast(Variable, self.v2).mark) != mark and cbool(stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v2).walk_strength))):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if int64(cast(Variable, self.v2).mark) == mark:
            if int64(cast(Variable, self.v1).mark) != mark and cbool(stronger(cast(Strength, self.strength), cast(Strength, cast(Variable, self.v1).walk_strength))):
                self.direction = Direction.BACKWARD
            else:
                self.direction = Direction.NONE
    ...
    def input(self):
        return cast(Variable, cast(Variable, self.v1) if cast(Direction, self.direction) == Direction.FORWARD else cast(Variable, self.v2))

    def output(self):
        return cast(Variable, cast(Variable, self.v2) if cast(Direction, self.direction) == Direction.FORWARD else cast(Variable, self.v1))
    ...
    def inputs_known(self, mark):
        ...
        return box(cbool(int64(i.mark) == mark or cbool(i.stay) or cbool(i.determined_by is None)))
...
class ScaleConstraint(BinaryConstraint):
    ...
    def mark_inputs(self, mark):
        BinaryConstraint.mark_inputs(box(int64(self)), mark)
...
class Planner(object):
    ...
    def incremental_add(self, constraint):
        ...
        overridden = cast(Constraint | Unknown, constraint.satisfy(box(int64(mark))))
        while overridden is not None:
            overridden = cast(Constraint | Unknown, overridden.satisfy(box(int64(mark))))
    ...
    def make_plan(self, sources):
        ...
        while clen(todo):
            c = todo.pop(0)
            if c.output().mark != int64(mark) and c.inputs_known(int64(mark)):
                plan.add_constraint(c)
                c.output().mark = int64(mark)
